/* Generated By:JavaCC: Do not edit this line. ZqlJJParser.java */
package anders.akita.parser;

import java.io.*;
import java.util.*;

/**
 * ZqlJJParser: an SQL parser
 */
public class ZqlJJParser implements ZqlJJParserConstants {

  public static void main(String args[]) throws ParseException {

    ZqlJJParser p = null ;

    if ( args.length < 1  ) {
      System.out.println("Reading from stdin (exit; to finish)");
      p = new ZqlJJParser(System.in) ;

    } else {

      try {
        p = new ZqlJJParser(new DataInputStream(
                          new FileInputStream(args[0]))) ;
      } catch (FileNotFoundException e) {
        System.out.println("File " + args[0] +
                           " not found. Reading from stdin") ;
        p = new ZqlJJParser(System.in) ;
      }
    } // else ends here

    if ( args.length > 0 ) {
      System.out.println(args[0]) ;
    }

    ZStatement st = null;
    while((st = p.SQLStatement()) != null) {
      System.out.println(st.toString());
    }
/*
    Vector v = p.SQLStatements();

    for(int i=0; i<v.size(); i++) {
      ZStatement st = (ZStatement)v.elementAt(i);
      System.out.println(st.toString());
    }
*/

    System.out.println("Parse Successful") ;

  }

  final public void BasicDataTypeDeclaration() throws ParseException {
    switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
    case K_CHAR:
    case K_FLOAT:
    case K_INTEGER:
    case K_NATURAL:
    case K_NUMBER:
    case K_REAL:
    case K_VARCHAR2:
    case K_VARCHAR:
      switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
      case K_CHAR:
        jj_consume_token(K_CHAR);
        break;
      case K_VARCHAR:
        jj_consume_token(K_VARCHAR);
        break;
      case K_VARCHAR2:
        jj_consume_token(K_VARCHAR2);
        break;
      case K_INTEGER:
        jj_consume_token(K_INTEGER);
        break;
      case K_NUMBER:
        jj_consume_token(K_NUMBER);
        break;
      case K_NATURAL:
        jj_consume_token(K_NATURAL);
        break;
      case K_REAL:
        jj_consume_token(K_REAL);
        break;
      case K_FLOAT:
        jj_consume_token(K_FLOAT);
        break;
      default:
        jj_la1[0] = jj_gen;
        jj_consume_token(-1);
        throw new ParseException();
      }
      switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
      case 115:
        jj_consume_token(115);
        jj_consume_token(S_NUMBER);
        switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
        case 116:
          jj_consume_token(116);
          jj_consume_token(S_NUMBER);
          break;
        default:
          jj_la1[1] = jj_gen;
          ;
        }
        jj_consume_token(117);
        break;
      default:
        jj_la1[2] = jj_gen;
        ;
      }
      break;
    case K_DATE:
      jj_consume_token(K_DATE);
      break;
    case K_BINARY_INTEGER:
      jj_consume_token(K_BINARY_INTEGER);
      break;
    case K_BOOLEAN:
      jj_consume_token(K_BOOLEAN);
      break;
    default:
      jj_la1[3] = jj_gen;
      jj_consume_token(-1);
      throw new ParseException();
    }
  }

/* ---------------- DECLARATIONS SECTION ends here ------------------ */

/* ---------------- Code Section starts here ---------------------- */
  final public Vector SQLStatements() throws ParseException {
  Vector v = new Vector();
  ZStatement s;
    label_1:
    while (true) {
      s = SQLStatement();
                          if(s == null) {if (true) return v;} else v.addElement(s);
      switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
      case K_EXIT:
      case K_QUIT:
      case K_SELECT:
        ;
        break;
      default:
        jj_la1[4] = jj_gen;
        break label_1;
      }
    }
      {if (true) return v;}
    throw new Error("Missing return statement in function");
  }

  final public ZStatement SQLStatement() throws ParseException {
  ZStatement s = null;
    switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
    case K_SELECT:
      s = QueryStatement();
                           {if (true) return s;}
      break;
    case K_EXIT:
    case K_QUIT:
      switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
      case K_EXIT:
        jj_consume_token(K_EXIT);
        break;
      case K_QUIT:
        jj_consume_token(K_QUIT);
        break;
      default:
        jj_la1[5] = jj_gen;
        jj_consume_token(-1);
        throw new ParseException();
      }
      jj_consume_token(118);
                              {if (true) return null;}
      break;
    default:
      jj_la1[6] = jj_gen;
      jj_consume_token(-1);
      throw new ParseException();
    }
    throw new Error("Missing return statement in function");
  }

  final public String identifier() throws ParseException {
        Token tk;
    tk = jj_consume_token(S_IDENTIFIER);
         {if (true) return tk.toString().toUpperCase();}
    throw new Error("Missing return statement in function");
  }

  final public ZUdf SQLUdf() throws ParseException {
        String s;
        ZUdf udf;
        ArrayList<String> parm = new ArrayList<String>();
        String funcName;
        ZExp exp;
    if (jj_2_1(2147483647)) {
                         {if (true) return null;}
    } else {
      switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
      case S_IDENTIFIER:
        funcName = identifier();
        jj_consume_token(115);
        switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
        case S_IDENTIFIER:
          s = identifier();
                                          parm.add(s);
          label_2:
          while (true) {
            switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
            case 116:
              ;
              break;
            default:
              jj_la1[7] = jj_gen;
              break label_2;
            }
            jj_consume_token(116);
            s = identifier();
                                                  parm.add(s);
          }
          break;
        default:
          jj_la1[8] = jj_gen;
          ;
        }
        jj_consume_token(117);
        jj_consume_token(119);
        exp = SQLExpression();
        jj_consume_token(118);
                  {if (true) return new ZUdf( funcName, parm, exp );}
        break;
      default:
        jj_la1[9] = jj_gen;
        jj_consume_token(-1);
        throw new ParseException();
      }
    }
    throw new Error("Missing return statement in function");
  }

// CommitStatement ::= COMMIT [WORK] [COMMENT <comment>];
  final public ZTransactStmt CommitStatement() throws ParseException {
  Token tk;
  ZTransactStmt t = new ZTransactStmt("COMMIT");
    jj_consume_token(K_COMMIT);
    switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
    case K_WORK:
      jj_consume_token(K_WORK);
      break;
    default:
      jj_la1[10] = jj_gen;
      ;
    }
    switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
    case K_COMMENT:
      jj_consume_token(K_COMMENT);
      tk = jj_consume_token(S_CHAR_LITERAL);
                                     t.setComment(tk.toString());
      break;
    default:
      jj_la1[11] = jj_gen;
      ;
    }
    jj_consume_token(118);
    {if (true) return t;}
    throw new Error("Missing return statement in function");
  }

// LockTableStatement ::= LOCK TABLE TableReference (, TableReference)*
//   IN LockMode MODE [NOWAIT];
  final public ZLockTable LockTableStatement() throws ParseException {
  ZLockTable lck = new ZLockTable();
  Vector v = new Vector();
  String s;
    jj_consume_token(K_LOCK);
    jj_consume_token(K_TABLE);
    s = TableReference();
                                        v.addElement(s);
    label_3:
    while (true) {
      switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
      case 116:
        ;
        break;
      default:
        jj_la1[12] = jj_gen;
        break label_3;
      }
      jj_consume_token(116);
      s = TableReference();
                              v.addElement(s);
    }
    jj_consume_token(K_IN);
    s = LockMode();
                        lck.setLockMode(s);
    jj_consume_token(K_MODE);
    switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
    case K_NOWAIT:
      jj_consume_token(K_NOWAIT);
              lck.nowait_ = true;
      break;
    default:
      jj_la1[13] = jj_gen;
      ;
    }
    jj_consume_token(118);
    lck.addTables(v); {if (true) return lck;}
    throw new Error("Missing return statement in function");
  }

// RollbackStatement ::= ROLLBACK [WORK] ["COMMENT" <comment>];
  final public ZTransactStmt RollbackStatement() throws ParseException {
  Token tk;
  ZTransactStmt t = new ZTransactStmt("ROLLBACK");
    jj_consume_token(K_ROLLBACK);
    switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
    case K_WORK:
      jj_consume_token(K_WORK);
      break;
    default:
      jj_la1[14] = jj_gen;
      ;
    }
    switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
    case K_COMMENT:
      jj_consume_token(K_COMMENT);
      tk = jj_consume_token(S_CHAR_LITERAL);
                                     t.setComment(tk.toString());
      break;
    default:
      jj_la1[15] = jj_gen;
      ;
    }
    jj_consume_token(118);
    {if (true) return t;}
    throw new Error("Missing return statement in function");
  }

// SetTransactionStatement ::= SET TRANSACTION READ (ONLY | WRITE)
  final public ZTransactStmt SetTransactionStatement() throws ParseException {
  StringBuffer b;
  ZTransactStmt t = new ZTransactStmt("SET TRANSACTION");
  boolean rdonly = false;
    jj_consume_token(K_SET);
    jj_consume_token(K_TRANSACTION);
    jj_consume_token(K_READ);
    switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
    case K_ONLY:
      jj_consume_token(K_ONLY);
                                        rdonly = true;
      break;
    case K_WRITE:
      jj_consume_token(K_WRITE);
      break;
    default:
      jj_la1[16] = jj_gen;
      jj_consume_token(-1);
      throw new ParseException();
    }
    jj_consume_token(118);
    t.readOnly_ = rdonly; {if (true) return t;}
    throw new Error("Missing return statement in function");
  }

// LockMode ::= ROW SHARE | ROW EXCLUSIVE
//   | SHARE [UPDATE | ROW EXCLUSIVE]
//   | EXCLUSIVE
  final public String LockMode() throws ParseException {
  StringBuffer b = new StringBuffer();
    switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
    case K_ROW:
      jj_consume_token(K_ROW);
            b.append("ROW ");
      switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
      case K_SHARE:
        jj_consume_token(K_SHARE);
               b.append("SHARE");
        break;
      case K_EXCLUSIVE:
        jj_consume_token(K_EXCLUSIVE);
                                                    b.append("EXCLUSIVE");
        break;
      default:
        jj_la1[17] = jj_gen;
        jj_consume_token(-1);
        throw new ParseException();
      }
     {if (true) return b.toString();}
      break;
    case K_SHARE:
      jj_consume_token(K_SHARE);
              b.append("SHARE");
      switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
      case K_ROW:
      case K_UPDATE:
        switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
        case K_UPDATE:
          jj_consume_token(K_UPDATE);
                b.append(" UPDATE");
          break;
        case K_ROW:
          jj_consume_token(K_ROW);
          jj_consume_token(K_EXCLUSIVE);
                             b.append(" ROW EXCLUSIVE");
          break;
        default:
          jj_la1[18] = jj_gen;
          jj_consume_token(-1);
          throw new ParseException();
        }
        break;
      default:
        jj_la1[19] = jj_gen;
        ;
      }
     {if (true) return b.toString();}
      break;
    case K_EXCLUSIVE:
      jj_consume_token(K_EXCLUSIVE);
                   {if (true) return new String("EXCLUSIVE");}
      break;
    default:
      jj_la1[20] = jj_gen;
      jj_consume_token(-1);
      throw new ParseException();
    }
    throw new Error("Missing return statement in function");
  }

// UpdateStatement ::=
//   UPDATE TableReference SET ColumnValues [WHERE SQLExpression]
  final public ZUpdate UpdateStatement() throws ParseException {
  ZUpdate u;
  ZExp e;
  Hashtable t;
  String s;
  Token tk;
    jj_consume_token(K_UPDATE);
    s = TableReference();
                                  u = new ZUpdate(s);
    switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
    case S_IDENTIFIER:
      s = identifier();
                       u.setAlias(s);
      break;
    default:
      jj_la1[21] = jj_gen;
      ;
    }
    jj_consume_token(K_SET);
    ColumnValues(u);

    switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
    case K_WHERE:
      jj_consume_token(K_WHERE);
      e = SQLExpression();
                                  u.addWhere(e);
      break;
    default:
      jj_la1[22] = jj_gen;
      ;
    }
    jj_consume_token(118);
    {if (true) return u;}
    throw new Error("Missing return statement in function");
  }

// ColumnValues ::= TableColumn = UpdatedValue (, TableColumn = UpdatedValue)*
  final public void ColumnValues(ZUpdate u) throws ParseException {
  ZColRef key;
  ZExp val;
    key = TableColumn();
    jj_consume_token(120);
    val = UpdatedValue();
                                                 u.addColumnUpdate(key.toString(), val);
    label_4:
    while (true) {
      switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
      case 116:
        ;
        break;
      default:
        jj_la1[23] = jj_gen;
        break label_4;
      }
      jj_consume_token(116);
      key = TableColumn();
      jj_consume_token(120);
      val = UpdatedValue();
      u.addColumnUpdate(key.toString(), val);
    }

  }

// UpdatedValue ::= ( SelectStatement ) | SQLExpression
  final public ZExp UpdatedValue() throws ParseException {
  ZExp e;
    if (jj_2_2(2147483647)) {
      jj_consume_token(115);
      e = SelectStatement();
      jj_consume_token(117);
                                  {if (true) return e;}
    } else {
      switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
      case K_AVG:
      case K_CASE:
      case K_COUNT:
      case K_DATE:
      case K_DAY:
      case K_EXISTS:
      case K_FALSE:
      case K_HOUR:
      case K_INTERVAL:
      case K_LEFT:
      case K_MAX:
      case K_MIN:
      case K_MOD:
      case K_MINUTE:
      case K_MONTH:
      case K_NOT:
      case K_NULL:
      case K_QUARTER:
      case K_RIGHT:
      case K_SECOND:
      case K_SUM:
      case K_TRUE:
      case K_WEEK:
      case K_YEAR:
      case S_NUMBER:
      case S_IDENTIFIER:
      case S_CHAR_LITERAL:
      case 115:
      case 138:
      case 139:
      case 143:
      case 144:
        e = SQLExpression();
                        {if (true) return e;}
        break;
      case 133:
        e = PreparedCol();
                      {if (true) return e;}
        break;
      default:
        jj_la1[24] = jj_gen;
        jj_consume_token(-1);
        throw new ParseException();
      }
    }
    throw new Error("Missing return statement in function");
  }

// InsertStatement ::= INSERT INTO TableReference
//   [ "(" TableColumn (, TableColumn)* ")" ]
//   ValueSpec | SelectStatement
// ValueSpec ::= VALUES ( SQLExpressionList )
  final public ZInsert InsertStatement() throws ParseException {
  ZInsert ins;
  String s;
  ZColRef col;
  Vector v, v2;
    jj_consume_token(K_INSERT);
    jj_consume_token(K_INTO);
    s = TableReference();
                                         ins = new ZInsert(s);
    switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
    case 115:
      jj_consume_token(115);
      col = TableColumn();
                              v = new Vector(); v.addElement(s);
      label_5:
      while (true) {
        switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
        case 116:
          ;
          break;
        default:
          jj_la1[25] = jj_gen;
          break label_5;
        }
        jj_consume_token(116);
        col = TableColumn();
                               v.addElement(s);
      }
      jj_consume_token(117);
                                                           ins.setColumns(v);
      break;
    default:
      jj_la1[26] = jj_gen;
      ;
    }
    jj_consume_token(K_VALUES);
    jj_consume_token(115);
    v2 = SQLExpressionList();
    jj_consume_token(117);
        ins.setValueSpec(v2);
    jj_consume_token(118);
         {if (true) return ins;}
    throw new Error("Missing return statement in function");
  }

// DeleteStatement ::= DELETE [FROM] TableReference [WHERE SQLExpression]
  final public ZDelete DeleteStatement() throws ParseException {
  ZDelete d;
  ZExp e;
  String s;
    jj_consume_token(K_DELETE);
    switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
    case K_FROM:
      jj_consume_token(K_FROM);
      break;
    default:
      jj_la1[27] = jj_gen;
      ;
    }
    s = TableReference();
                                             d = new ZDelete(s);
    switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
    case K_WHERE:
      jj_consume_token(K_WHERE);
      e = SQLExpression();
                                   d.addWhere(e);
      break;
    default:
      jj_la1[28] = jj_gen;
      ;
    }
    jj_consume_token(118);
      {if (true) return d;}
    throw new Error("Missing return statement in function");
  }

// QueryStatement ::= SelectStatement
  final public ZQuery QueryStatement() throws ParseException {
  ZQuery q;
    q = SelectStatement();
    jj_consume_token(118);
                                {if (true) return q;}
    throw new Error("Missing return statement in function");
  }

/* ---------------- General Productions --------------------- */
  final public ZColRef TableColumn() throws ParseException {
  String t = null, c;
    if (jj_2_3(2)) {
      t = identifier();
      jj_consume_token(121);
      c = identifier();
    } else {
      switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
      case S_IDENTIFIER:
        c = identifier();
        break;
      default:
        jj_la1[29] = jj_gen;
        jj_consume_token(-1);
        throw new ParseException();
      }
    }
      {if (true) return new ZColRef(t, c);}
    throw new Error("Missing return statement in function");
  }

// TableReference ::= OracleObjectName[.OracleObjectName]
  final public String TableReference() throws ParseException {
  String s;
    s = identifier();
                       {if (true) return s;}
    throw new Error("Missing return statement in function");
  }

  final public ZQuery SelectStatement() throws ParseException {
  ZQuery q;
  Vector v;
    q = SelectWithoutOrder();
    switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
    case K_ORDER:
      v = OrderByClause();
                          q.setOrderBy(v);
      break;
    default:
      jj_la1[30] = jj_gen;
      ;
    }
    {if (true) return q;}
    throw new Error("Missing return statement in function");
  }

  final public ZQuery SelectWithoutOrder() throws ParseException {
  ZQuery q = new ZQuery();
  Vector select;
  ZFromClause from;
  ZExp where = null;
  ZGroupBy groupby = null;
    jj_consume_token(K_SELECT);
    switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
    case K_ALL:
    case K_DISTINCT:
      switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
      case K_ALL:
        jj_consume_token(K_ALL);
        break;
      case K_DISTINCT:
        jj_consume_token(K_DISTINCT);
                                  q.distinct_ = true;
        break;
      default:
        jj_la1[31] = jj_gen;
        jj_consume_token(-1);
        throw new ParseException();
      }
      break;
    default:
      jj_la1[32] = jj_gen;
      ;
    }
    select = SelectList();
    from = FromClause();
    switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
    case K_WHERE:
      where = WhereClause();
      break;
    default:
      jj_la1[33] = jj_gen;
      ;
    }
    if (jj_2_4(2147483647)) {
      groupby = GroupByClause();
    } else {
      ;
    }
    q.setSelect(select);
    q.setFrom(from);
    q.setWhere(where);
    q.setGroupBy(groupby);
    //q.addSet(setclause);

    {if (true) return q;}
    throw new Error("Missing return statement in function");
  }

/* Checks for whatever follows  SELECT */

// SelectList ::= * | COUNT(*) | SelectItem (, SelectItem)*
// 18/10/01 PY. Gibello - Removed "COUNT(*)" from here
// COUNT is an aggregate, like AVG...
  final public Vector SelectList() throws ParseException {
  Vector v = new Vector(8);
  ZSelectItem elem;
    elem = SelectItem();
                          v.addElement(elem);
    label_6:
    while (true) {
      switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
      case 116:
        ;
        break;
      default:
        jj_la1[34] = jj_gen;
        break label_6;
      }
      jj_consume_token(116);
      elem = SelectItem();
                               v.addElement(elem);
    }
        int star_cnt = 0;
        for(Object e: v){
                if(((ZSelectItem)e).type == ZSelectItem.STAR)
                        ++star_cnt;
        }
        if(star_cnt > 1)
                {if (true) throw new ParseException("Parse error: not permit multiple select *");}
        {if (true) return v;}
    throw new Error("Missing return statement in function");
  }

// 01/07/00 PY. Gibello - Added LOOKAHEAD(3) before COUNT
// Due to a fix in AggregateFunc (COUNT added) that made the grammar ambiguous
// 18/10/01 PY. Gibello - Removed "COUNT(*)" from here (see aggregate funcs)

// SelectItem() ::= SelectStar | SQLSimpleExpression() [SelectAlias]
  final public ZSelectItem SelectItem() throws ParseException {
  String s = null;
  ZSelectItem it;
  ZExp e;
    switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
    case 122:
      jj_consume_token(122);
                           {if (true) return new ZSelectItem();}
      break;
    default:
      jj_la1[37] = jj_gen;
      if (jj_2_5(2)) {
        s = identifier();
        jj_consume_token(123);
                                            {if (true) return new ZSelectItem(s);}
      } else {
        switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
        case K_AVG:
        case K_CASE:
        case K_COUNT:
        case K_DATE:
        case K_DAY:
        case K_EXISTS:
        case K_FALSE:
        case K_HOUR:
        case K_INTERVAL:
        case K_LEFT:
        case K_MAX:
        case K_MIN:
        case K_MOD:
        case K_MINUTE:
        case K_MONTH:
        case K_NOT:
        case K_NULL:
        case K_QUARTER:
        case K_RIGHT:
        case K_SECOND:
        case K_SUM:
        case K_TRUE:
        case K_WEEK:
        case K_YEAR:
        case S_NUMBER:
        case S_IDENTIFIER:
        case S_CHAR_LITERAL:
        case 115:
        case 138:
        case 139:
        case 143:
        case 144:
          e = SQLExpression();
          switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
          case K_AS:
          case S_IDENTIFIER:
            switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
            case K_AS:
              jj_consume_token(K_AS);
              break;
            default:
              jj_la1[35] = jj_gen;
              ;
            }
            s = identifier();
            break;
          default:
            jj_la1[36] = jj_gen;
            ;
          }
      {if (true) return new ZSelectItem(e, s);}
          break;
        default:
          jj_la1[38] = jj_gen;
          jj_consume_token(-1);
          throw new ParseException();
        }
      }
    }
    throw new Error("Missing return statement in function");
  }

  final public ZFromClause FromClause() throws ParseException {
        ZFromClause fc;
    jj_consume_token(K_FROM);
    if (jj_2_6(2147483647)) {
      fc = LeftOuterJoinClause();
    } else {
      switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
      case S_IDENTIFIER:
      case 115:
        fc = InnerJoinClause();
        break;
      default:
        jj_la1[39] = jj_gen;
        jj_consume_token(-1);
        throw new ParseException();
      }
    }
         {if (true) return fc;}
    throw new Error("Missing return statement in function");
  }

// FromClause ::= FromItem (, FromItem)*
  final public ZFromClause InnerJoinClause() throws ParseException {
  Vector v = new Vector(8);
  ZFromItemEx f;
    f = FromItemEx();
                       v.addElement(f);
    label_7:
    while (true) {
      switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
      case 116:
        ;
        break;
      default:
        jj_la1[40] = jj_gen;
        break label_7;
      }
      jj_consume_token(116);
      f = FromItemEx();
                             v.addElement(f);
    }
      {if (true) return new ZFromClause(ZFromClause.INNER_JOIN, v, null);}
    throw new Error("Missing return statement in function");
  }

  final public ZFromClause LeftOuterJoinClause() throws ParseException {
        ZFromClause f;
        ZFromItemEx il, ir;
        ZExp cond = null;
        int join_type;
    il = FromItemEx();
    switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
    case K_LEFT:
      jj_consume_token(K_LEFT);
                 join_type = ZFromClause.LEFT_JOIN;
      break;
    case K_RIGHT:
      jj_consume_token(K_RIGHT);
                                                              join_type = ZFromClause.RIGHT_JOIN;
      break;
    default:
      jj_la1[41] = jj_gen;
      jj_consume_token(-1);
      throw new ParseException();
    }
    switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
    case K_OUTER:
      jj_consume_token(K_OUTER);
      break;
    default:
      jj_la1[42] = jj_gen;
      ;
    }
    jj_consume_token(K_JOIN);
    ir = FromItemEx();
    switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
    case K_ON:
      jj_consume_token(K_ON);
      cond = SQLExpression();
      break;
    default:
      jj_la1[43] = jj_gen;
      ;
    }
                Vector v = new Vector(2);
                v.addElement(il);
                v.addElement(ir);
                {if (true) return new ZFromClause(join_type, v, cond);}
    throw new Error("Missing return statement in function");
  }

// FromItemEx ::= ( FromItem | "(" SelectStatement ")" ) [ "AS" alias ]
  final public ZFromItemEx FromItemEx() throws ParseException {
        ZFromItemEx f = null;
        ZQuery subq = null;
        ZFromItem tab_item = null;
        String tab = null;
        String s = null;
    switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
    case S_IDENTIFIER:
      tab = TableReference();
                                    tab_item = new ZFromItem(tab); f = new ZFromItemEx(tab_item);
      break;
    case 115:
      jj_consume_token(115);
      subq = SelectStatement();
      jj_consume_token(117);
                                               f = new ZFromItemEx(subq);
      break;
    default:
      jj_la1[44] = jj_gen;
      jj_consume_token(-1);
      throw new ParseException();
    }
    switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
    case K_AS:
    case S_IDENTIFIER:
      switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
      case K_AS:
        jj_consume_token(K_AS);
        break;
      default:
        jj_la1[45] = jj_gen;
        ;
      }
      s = identifier();
                 f.alias = s;
      break;
    default:
      jj_la1[46] = jj_gen;
      ;
    }
         {if (true) return f;}
    throw new Error("Missing return statement in function");
  }

  final public ZExp WhereClause() throws ParseException {
  ZExp e;
    jj_consume_token(K_WHERE);
    e = SQLExpression();
                                  {if (true) return e;}
    throw new Error("Missing return statement in function");
  }

/*
void ConnectClause():
{}
{
    // The following grammar will take 2 "START WITH" expressions
    // which is not correct. But alright, because only valid statements
    // will be given.
   (["START" "WITH" SQLExpression()] "CONNECT" "BY" SQLExpression()
    ["START" "WITH" SQLExpression()])
}
*/
  final public ZGroupBy GroupByClause() throws ParseException {
  ZGroupBy g = null;
  Vector v = null;
  ZExp e;
    switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
    case K_GROUP:
      jj_consume_token(K_GROUP);
      jj_consume_token(K_BY);
      v = GroupByList();
                                     g = new ZGroupBy(v);
      switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
      case K_HAVING:
        jj_consume_token(K_HAVING);
        e = SQLExpression();
                                         g.setHaving(e);
        break;
      default:
        jj_la1[47] = jj_gen;
        ;
      }
      break;
    case K_HAVING:
      jj_consume_token(K_HAVING);
      e = SQLExpression();
                                   g = new ZGroupBy(null); g.setHaving(e);
      break;
    default:
      jj_la1[48] = jj_gen;
      jj_consume_token(-1);
      throw new ParseException();
    }
    {if (true) return g;}
    throw new Error("Missing return statement in function");
  }

  final public Vector GroupByList() throws ParseException {
  Vector v = new Vector(8);
  ZExp e;
    e = SQLExpression();
                          v.addElement(e);
    label_8:
    while (true) {
      switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
      case 116:
        ;
        break;
      default:
        jj_la1[49] = jj_gen;
        break label_8;
      }
      jj_consume_token(116);
      e = TableColumn();
                             v.addElement(e);
    }
      {if (true) return v;}
    throw new Error("Missing return statement in function");
  }

  final public Vector OrderByClause() throws ParseException {
  Vector v = new Vector();
  ZExp s;
  ZOrderBy ob;
    jj_consume_token(K_ORDER);
    jj_consume_token(K_BY);
    s = SQLExpression();
                                       ob = new ZOrderBy(s);
    switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
    case K_ASC:
    case K_DESC:
      switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
      case K_ASC:
        jj_consume_token(K_ASC);
        break;
      case K_DESC:
        jj_consume_token(K_DESC);
                      ob.setAscOrder(false);
        break;
      default:
        jj_la1[50] = jj_gen;
        jj_consume_token(-1);
        throw new ParseException();
      }
      break;
    default:
      jj_la1[51] = jj_gen;
      ;
    }
                                                  v.addElement(ob);
    label_9:
    while (true) {
      switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
      case 116:
        ;
        break;
      default:
        jj_la1[52] = jj_gen;
        break label_9;
      }
      jj_consume_token(116);
      s = SQLExpression();
                               ob = new ZOrderBy(s);
      switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
      case K_ASC:
      case K_DESC:
        switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
        case K_ASC:
          jj_consume_token(K_ASC);
          break;
        case K_DESC:
          jj_consume_token(K_DESC);
                       ob.setAscOrder(false);
          break;
        default:
          jj_la1[53] = jj_gen;
          jj_consume_token(-1);
          throw new ParseException();
        }
        break;
      default:
        jj_la1[54] = jj_gen;
        ;
      }
                                                   v.addElement(ob);
    }
      {if (true) return v;}
    throw new Error("Missing return statement in function");
  }

/*
void ForUpdateClause():
{}
{
    "FOR" "UPDATE" [ "OF" TableColumn() ("," TableColumn())* ]
}
*/
  final public ZExp SQLExpression() throws ParseException {
  ZExp e1, e2;
  ZExpression e = null;
  boolean single = true;
    e1 = SQLXORExpression();
    label_10:
    while (true) {
      switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
      case K_OR:
      case 124:
        ;
        break;
      default:
        jj_la1[55] = jj_gen;
        break label_10;
      }
      switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
      case K_OR:
        jj_consume_token(K_OR);
        break;
      case 124:
        jj_consume_token(124);
        break;
      default:
        jj_la1[56] = jj_gen;
        jj_consume_token(-1);
        throw new ParseException();
      }
      e2 = SQLXORExpression();
      if(single) { e = new ZExpression(Operator.OR, e1); }
      single=false;
      e.addOperand(e2);
    }
      {if (true) return (single ? e1 : e);}
    throw new Error("Missing return statement in function");
  }

  final public ZExp SQLXORExpression() throws ParseException {
  ZExp e1, e2;
  ZExpression e = null;
  boolean single = true;
    e1 = SQLAndExpression();
    label_11:
    while (true) {
      switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
      case K_XOR:
        ;
        break;
      default:
        jj_la1[57] = jj_gen;
        break label_11;
      }
      jj_consume_token(K_XOR);
      e2 = SQLAndExpression();
      if(single) { e = new ZExpression(Operator.XOR, e1); }
      single=false;
      e.addOperand(e2);
    }
      {if (true) return (single ? e1 : e);}
    throw new Error("Missing return statement in function");
  }

  final public ZExp SQLAndExpression() throws ParseException {
  ZExp e1, e2;
  ZExpression e = null;
  boolean single = true;
    e1 = SQLUnaryLogicalExpression();
    label_12:
    while (true) {
      switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
      case K_AND:
      case 125:
        ;
        break;
      default:
        jj_la1[58] = jj_gen;
        break label_12;
      }
      switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
      case K_AND:
        jj_consume_token(K_AND);
        break;
      case 125:
        jj_consume_token(125);
        break;
      default:
        jj_la1[59] = jj_gen;
        jj_consume_token(-1);
        throw new ParseException();
      }
      e2 = SQLUnaryLogicalExpression();
      if(single) { e = new ZExpression(Operator.AND, e1); }
      single=false;
      e.addOperand(e2);
    }
      {if (true) return (single ? e1 : e);}
    throw new Error("Missing return statement in function");
  }

  final public ZExp SQLUnaryLogicalExpression() throws ParseException {
  ZExp e1, e;
  boolean not = false;
    if (jj_2_7(2)) {
      e = ExistsClause();
                                      {if (true) return e;}
    } else {
      switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
      case K_AVG:
      case K_CASE:
      case K_COUNT:
      case K_DATE:
      case K_DAY:
      case K_FALSE:
      case K_HOUR:
      case K_INTERVAL:
      case K_LEFT:
      case K_MAX:
      case K_MIN:
      case K_MOD:
      case K_MINUTE:
      case K_MONTH:
      case K_NOT:
      case K_NULL:
      case K_QUARTER:
      case K_RIGHT:
      case K_SECOND:
      case K_SUM:
      case K_TRUE:
      case K_WEEK:
      case K_YEAR:
      case S_NUMBER:
      case S_IDENTIFIER:
      case S_CHAR_LITERAL:
      case 115:
      case 138:
      case 139:
      case 143:
      case 144:
        switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
        case K_NOT:
          jj_consume_token(K_NOT);
               not = true;
          break;
        default:
          jj_la1[60] = jj_gen;
          ;
        }
        e1 = SQLConditionL1Expression();
       if(not) e = new ZExpression(Operator.NOT, e1);
       else e = e1;
       {if (true) return e;}
        break;
      default:
        jj_la1[61] = jj_gen;
        jj_consume_token(-1);
        throw new ParseException();
      }
    }
    throw new Error("Missing return statement in function");
  }

  final public ZExp ExistsClause() throws ParseException {
  ZQuery q;
  boolean not = false;
    switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
    case K_NOT:
      jj_consume_token(K_NOT);
             not = true;
      break;
    default:
      jj_la1[62] = jj_gen;
      ;
    }
    jj_consume_token(K_EXISTS);
    q = SQLInnerQuery(ZQuery.InnerQType.EXISTS);
      {if (true) return new ZExpression(not ? Operator.NOT_EXISTS : Operator.EXISTS, q);}
    throw new Error("Missing return statement in function");
  }

  final public ZExp SQLConditionL1Expression() throws ParseException {
        ZExp e;
    if (jj_2_8(2147483647)) {
      e = SQLSwitchClause();
                                                 {if (true) return e;}
    } else if (jj_2_9(2147483647)) {
      e = SQLBetweenClause();
                                                                                         {if (true) return e;}
    } else {
      switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
      case K_AVG:
      case K_COUNT:
      case K_DATE:
      case K_DAY:
      case K_FALSE:
      case K_HOUR:
      case K_INTERVAL:
      case K_LEFT:
      case K_MAX:
      case K_MIN:
      case K_MOD:
      case K_MINUTE:
      case K_MONTH:
      case K_NULL:
      case K_QUARTER:
      case K_RIGHT:
      case K_SECOND:
      case K_SUM:
      case K_TRUE:
      case K_WEEK:
      case K_YEAR:
      case S_NUMBER:
      case S_IDENTIFIER:
      case S_CHAR_LITERAL:
      case 115:
      case 138:
      case 139:
      case 143:
      case 144:
        e = SQLConditionL2Expression();
                                         {if (true) return e;}
        break;
      default:
        jj_la1[63] = jj_gen;
        jj_consume_token(-1);
        throw new ParseException();
      }
    }
    throw new Error("Missing return statement in function");
  }

  final public ZExp SQLSwitchClause() throws ParseException {
        ZExp e, cmpVal = null, ce, re;
        ZExp else_result = null;
        Vector cond = new Vector(), result = new Vector() ;
    jj_consume_token(K_CASE);
    switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
    case K_AVG:
    case K_COUNT:
    case K_DATE:
    case K_DAY:
    case K_FALSE:
    case K_HOUR:
    case K_INTERVAL:
    case K_LEFT:
    case K_MAX:
    case K_MIN:
    case K_MOD:
    case K_MINUTE:
    case K_MONTH:
    case K_NULL:
    case K_QUARTER:
    case K_RIGHT:
    case K_SECOND:
    case K_SUM:
    case K_TRUE:
    case K_WEEK:
    case K_YEAR:
    case S_NUMBER:
    case S_IDENTIFIER:
    case S_CHAR_LITERAL:
    case 115:
    case 138:
    case 139:
    case 143:
    case 144:
      cmpVal = SQLConditionL2Expression();
      break;
    default:
      jj_la1[64] = jj_gen;
      ;
    }
    label_13:
    while (true) {
      jj_consume_token(K_WHEN);
      ce = SQLConditionL2Expression();
      jj_consume_token(K_THEN);
      re = SQLConditionL2Expression();
           cond.addElement(ce); result.addElement(re);
      switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
      case K_WHEN:
        ;
        break;
      default:
        jj_la1[65] = jj_gen;
        break label_13;
      }
    }
    switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
    case K_ELSE:
      jj_consume_token(K_ELSE);
      else_result = SQLConditionL2Expression();
      break;
    default:
      jj_la1[66] = jj_gen;
      ;
    }
    jj_consume_token(K_END);
         {if (true) return new ZSwitchExpr(cond, result, cmpVal, else_result);}
    throw new Error("Missing return statement in function");
  }

  final public ZExp SQLBetweenClause() throws ParseException {
  ZExp  e0, e1, e2;
  boolean not = false;
    e0 = SQLConditionL2Expression();
    switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
    case K_NOT:
      jj_consume_token(K_NOT);
           not = true;
      break;
    default:
      jj_la1[67] = jj_gen;
      ;
    }
    jj_consume_token(K_BETWEEN);
    e1 = SQLConditionL2Expression();
    jj_consume_token(K_AND);
    e2 = SQLConditionL2Expression();
    {if (true) return new ZExpression( not ? Operator.NOT_BETWEEN : Operator.BETWEEN, e0, e1, e2);}
    throw new Error("Missing return statement in function");
  }

  final public Operator Relop() throws ParseException {
  Token op;
    switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
    case 120:
      op = jj_consume_token(120);
               {if (true) return Operator.EQ;}
      break;
    case 126:
      op = jj_consume_token(126);
               {if (true) return Operator.NOT_EQ;}
      break;
    case 127:
      op = jj_consume_token(127);
               {if (true) return Operator.SAFE_EQ;}
      break;
    case 128:
      op = jj_consume_token(128);
               {if (true) return Operator.NOT_EQ;}
      break;
    case 129:
      op = jj_consume_token(129);
               {if (true) return Operator.MORE;}
      break;
    case 130:
      op = jj_consume_token(130);
               {if (true) return Operator.MORE_EQ;}
      break;
    case 131:
      op = jj_consume_token(131);
               {if (true) return Operator.LESS;}
      break;
    case 132:
      op = jj_consume_token(132);
               {if (true) return Operator.LESS_EQ;}
      break;
    default:
      jj_la1[68] = jj_gen;
      jj_consume_token(-1);
      throw new ParseException();
    }
    throw new Error("Missing return statement in function");
  }

  final public ZExp SQLConditionL2Expression() throws ParseException {
        ZExp e;
    if (jj_2_10(2147483647)) {
      e = SQLAllAnySubquery();
    } else if (jj_2_11(2147483647)) {
      e = SQLInSubquery();
    } else if (jj_2_12(2147483647)) {
      e = SQLRelationalExpression();
    } else if (jj_2_13(2147483647)) {
      e = SQLIsNullExpression();
    } else if (jj_2_14(2147483647)) {
      e = SQLLikeExpression();
    } else if (jj_2_15(2147483647)) {
      e = SQLRegexpExpression();
    } else {
      switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
      case K_AVG:
      case K_COUNT:
      case K_DATE:
      case K_DAY:
      case K_FALSE:
      case K_HOUR:
      case K_INTERVAL:
      case K_LEFT:
      case K_MAX:
      case K_MIN:
      case K_MOD:
      case K_MINUTE:
      case K_MONTH:
      case K_NULL:
      case K_QUARTER:
      case K_RIGHT:
      case K_SECOND:
      case K_SUM:
      case K_TRUE:
      case K_WEEK:
      case K_YEAR:
      case S_NUMBER:
      case S_IDENTIFIER:
      case S_CHAR_LITERAL:
      case 115:
      case 138:
      case 139:
      case 143:
      case 144:
        e = SQLSimpleExpression();
        break;
      default:
        jj_la1[69] = jj_gen;
        jj_consume_token(-1);
        throw new ParseException();
      }
    }
         {if (true) return e;}
    throw new Error("Missing return statement in function");
  }

  final public ZExp SQLAllAnySubquery() throws ParseException {
        ZExp e1;
        ZQuery e2;
        Operator op;
        ZQuery.InnerQType type;
    e1 = SQLSimpleExpression();
    op = Relop();
    switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
    case K_ALL:
      jj_consume_token(K_ALL);
                type = ZQuery.InnerQType.ALL;
      break;
    case K_ANY:
    case K_SOME:
      switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
      case K_ANY:
        jj_consume_token(K_ANY);
        break;
      case K_SOME:
        jj_consume_token(K_SOME);
        break;
      default:
        jj_la1[70] = jj_gen;
        jj_consume_token(-1);
        throw new ParseException();
      }
                                                                  type = ZQuery.InnerQType.ANY;
      break;
    default:
      jj_la1[71] = jj_gen;
      jj_consume_token(-1);
      throw new ParseException();
    }
    e2 = SQLInnerQuery(type);
                if(op.equals("<=>"))
                        {if (true) throw new ParseException("Can't use <=> operator in ALL/ANY sub-Query");}
                {if (true) return new ZExpression(op, e1, e2);}
    throw new Error("Missing return statement in function");
  }

  final public ZExp SQLInSubquery() throws ParseException {
        ZExp e1;
        ZQuery e2;
        Token op;
        boolean not = false;
    e1 = SQLSimpleExpression();
    switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
    case K_NOT:
      jj_consume_token(K_NOT);
                                            not = true;
      break;
    default:
      jj_la1[72] = jj_gen;
      ;
    }
    jj_consume_token(K_IN);
    e2 = SQLInnerQuery(ZQuery.InnerQType.IN);
                {if (true) return new ZExpression( not? Operator.NOT_IN:Operator.IN, e1, e2);}
    throw new Error("Missing return statement in function");
  }

  final public ZExp SQLRelationalExpression() throws ParseException {
        ZExp e1, e2;
        Operator op;
    e1 = SQLSimpleExpression();
    op = Relop();
    e2 = SQLSimpleExpression();
                {if (true) return new ZExpression(op, e1, e2);}
    throw new Error("Missing return statement in function");
  }

  final public ZExpression SQLIsNullExpression() throws ParseException {
        ZExp e1;
  boolean not = false;
    e1 = SQLSimpleExpression();
    jj_consume_token(K_IS);
    switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
    case K_NOT:
      jj_consume_token(K_NOT);
                                           not = true;
      break;
    default:
      jj_la1[73] = jj_gen;
      ;
    }
    jj_consume_token(K_NULL);
    {if (true) return new ZExpression( not ? Operator.IS_NOT_NULL : Operator.IS_NULL, e1);}
    throw new Error("Missing return statement in function");
  }

  final public Vector SQLExpressionList() throws ParseException {
  Vector v = new Vector(8);
  ZExp e;
    e = SQLExpression();
                          v.addElement(e);
    label_14:
    while (true) {
      switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
      case 116:
        ;
        break;
      default:
        jj_la1[74] = jj_gen;
        break label_14;
      }
      jj_consume_token(116);
      e = SQLExpression();
                               v.addElement(e);
    }
      {if (true) return v;}
    throw new Error("Missing return statement in function");
  }

// For prepared columns ("?")
  final public ZExp PreparedCol() throws ParseException {
    jj_consume_token(133);
        {if (true) return new ZConstant(null, ZConstant.PREPARED_COL);}
    throw new Error("Missing return statement in function");
  }

  final public ZExpression SQLLikeExpression() throws ParseException {
  ZExp e1, e2;
  boolean not = false;
    e1 = SQLSimpleExpression();
    switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
    case K_NOT:
      jj_consume_token(K_NOT);
           not = true;
      break;
    default:
      jj_la1[75] = jj_gen;
      ;
    }
    jj_consume_token(K_LIKE);
    e2 = SQLSimpleExpression();
    {if (true) return new ZExpression( not ? Operator.NOT_LIKE : Operator.LIKE , e1, e2);}
    throw new Error("Missing return statement in function");
  }

  final public ZExpression SQLRegexpExpression() throws ParseException {
        ZExp e1, e2;
  boolean not = false;
    e1 = SQLSimpleExpression();
    switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
    case K_NOT:
      jj_consume_token(K_NOT);
                  not = true;
      break;
    default:
      jj_la1[76] = jj_gen;
      ;
    }
    switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
    case K_LIKE:
      jj_consume_token(K_LIKE);
      break;
    case K_REGEXP:
      jj_consume_token(K_REGEXP);
      break;
    default:
      jj_la1[77] = jj_gen;
      jj_consume_token(-1);
      throw new ParseException();
    }
    e2 = SQLSimpleExpression();
    {if (true) return new ZExpression( not ? Operator.NOT_RLIKE : Operator.RLIKE , e1, e2);}
    throw new Error("Missing return statement in function");
  }

  final public ZExp SQLSimpleExpression() throws ParseException {
  Token op;
  ZExp e1, e2;
  ZExpression e = null;
  boolean single = true;
    e1 = SQLBitAndExpression();
    label_15:
    while (true) {
      switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
      case 134:
        ;
        break;
      default:
        jj_la1[78] = jj_gen;
        break label_15;
      }
      op = jj_consume_token(134);
      e2 = SQLBitAndExpression();
      if(single) e = new ZExpression( Operator.BIT_OR, e1);
      single = false;
      e.addOperand(e2);
    }
      {if (true) return (single ? e1 : e);}
    throw new Error("Missing return statement in function");
  }

  final public ZExp SQLBitAndExpression() throws ParseException {
  Token op;
  ZExp e1, e2;
  ZExpression e = null;
  boolean single = true;
    e1 = SQLBitShiftExpression();
    label_16:
    while (true) {
      switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
      case 135:
        ;
        break;
      default:
        jj_la1[79] = jj_gen;
        break label_16;
      }
      op = jj_consume_token(135);
      e2 = SQLBitShiftExpression();
      if(single) e = new ZExpression( Operator.BIT_AND, e1);
      single = false;
      e.addOperand(e2);
    }
      {if (true) return (single ? e1 : e);}
    throw new Error("Missing return statement in function");
  }

  final public ZExp SQLBitShiftExpression() throws ParseException {
  Operator op;
  ZExp e1, e2;
  ZExpression e = null;
    e1 = SQLAddSubExpression();
    label_17:
    while (true) {
      switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
      case 136:
      case 137:
        ;
        break;
      default:
        jj_la1[80] = jj_gen;
        break label_17;
      }
      switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
      case 136:
        jj_consume_token(136);
                 op = Operator.BIT_SHL;
        break;
      case 137:
        jj_consume_token(137);
                                                   op = Operator.BIT_SHR;
        break;
      default:
        jj_la1[81] = jj_gen;
        jj_consume_token(-1);
        throw new ParseException();
      }
      e2 = SQLAddSubExpression();
                e = new ZExpression(op, e1);
                e.addOperand(e2);
                e1 = e;
    }
      {if (true) return e1;}
    throw new Error("Missing return statement in function");
  }

// SQLSimpleExpression
//    ::= SQLMultiplicativeExpression (OP SQLMultiplicativeExpression)*
// OP ::= + | - 
  final public ZExp SQLAddSubExpression() throws ParseException {
  Operator op;
  ZExp e1, e2;
  ZExpression e = null;
    e1 = SQLMultiplicativeExpression();
    label_18:
    while (true) {
      switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
      case 138:
      case 139:
        ;
        break;
      default:
        jj_la1[82] = jj_gen;
        break label_18;
      }
      switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
      case 138:
        jj_consume_token(138);
                op = Operator.ADD;
        break;
      case 139:
        jj_consume_token(139);
                                             op = Operator.SUB;
        break;
      default:
        jj_la1[83] = jj_gen;
        jj_consume_token(-1);
        throw new ParseException();
      }
      e2 = SQLMultiplicativeExpression();
                e = new ZExpression(op, e1);
                e.addOperand(e2);
                e1 = e;
    }
      {if (true) return e1;}
    throw new Error("Missing return statement in function");
  }

// SQLMultiplicativeExpression
//    ::= SQLExpotentExpression (OP SQLExpotentExpression)*
// OP ::= * | / | "DIV" | "MOD" | % |
  final public ZExp SQLMultiplicativeExpression() throws ParseException {
  Operator op;
  ZExp e1, e2;
  ZExpression e = null;
    e1 = SQLBitXORExpression();
    label_19:
    while (true) {
      switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
      case K_DIV:
      case K_MOD:
      case 122:
      case 140:
      case 141:
        ;
        break;
      default:
        jj_la1[84] = jj_gen;
        break label_19;
      }
      switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
      case 122:
        jj_consume_token(122);
                op = Operator.MUL;
        break;
      case 140:
        jj_consume_token(140);
                                             op = Operator.DIV;
        break;
      case 141:
        jj_consume_token(141);
                                                                          op = Operator.MOD;
        break;
      case K_DIV:
        jj_consume_token(K_DIV);
                                                                                                         op = Operator.DIV;
        break;
      case K_MOD:
        jj_consume_token(K_MOD);
                                                                                                                                        op = Operator.MOD;
        break;
      default:
        jj_la1[85] = jj_gen;
        jj_consume_token(-1);
        throw new ParseException();
      }
      e2 = SQLBitXORExpression();
                e = new ZExpression(op, e1);
                e.addOperand(e2);
                e1 = e;
    }
      {if (true) return e1;}
    throw new Error("Missing return statement in function");
  }

  final public ZExp SQLBitXORExpression() throws ParseException {
  Token op;
  ZExp e1, e2;
  ZExpression e = null;
  boolean single = true;
    e1 = SQLUnaryExpression();
    label_20:
    while (true) {
      switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
      case 142:
        ;
        break;
      default:
        jj_la1[86] = jj_gen;
        break label_20;
      }
      op = jj_consume_token(142);
      e2 = SQLUnaryExpression();
      if(single) e = new ZExpression( Operator.BIT_XOR, e1);
      single = false;
      e.addOperand(e2);
    }
      {if (true) return (single ? e1 : e);}
    throw new Error("Missing return statement in function");
  }

/*
ZConstant SQLNumber():
{
	boolean neg = false;
	Token  num;
}
{
	[ "+" |  "-" { neg = true;} ] num = <S_NUMBER> 
	{return new ZConstant( neg ? "-" + num.toString() : num.toString(), ZConstant.NUMBER); }
}
*/
// SQLUnaryExpression ::= [+|-] SQLInversionExpression
  final public ZExp SQLUnaryExpression() throws ParseException {
  ZExp e1;
  boolean neg = false;
  boolean inv = false;
    switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
    case 138:
    case 139:
    case 143:
      switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
      case 138:
        jj_consume_token(138);
        break;
      case 139:
        jj_consume_token(139);
                  neg = true;
        break;
      case 143:
        jj_consume_token(143);
                                        inv = true;
        break;
      default:
        jj_la1[87] = jj_gen;
        jj_consume_token(-1);
        throw new ParseException();
      }
      break;
    default:
      jj_la1[88] = jj_gen;
      ;
    }
    e1 = SQLNotHighPrecExpression();
    if (neg){
      {if (true) return (( e1 instanceof ZConstant && ((ZConstant)e1).getType() == ZConstant.NUMBER )?
        new ZConstant("-" + ((ZConstant)e1).getValue() ,  ZConstant.NUMBER )
        : new ZExpression(Operator.NEG, e1));}
    }else if (inv){
        {if (true) return new ZExpression(Operator.INV, e1);}
     }
     else {if (true) return e1;}
    throw new Error("Missing return statement in function");
  }

  final public ZExp SQLNotHighPrecExpression() throws ParseException {
  Token op = null;
  ZExp e;
    switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
    case 144:
      op = jj_consume_token(144);
      break;
    default:
      jj_la1[89] = jj_gen;
      ;
    }
    e = SQLPrimaryExpression();
    if(op == null ) {if (true) return e;}
    else {if (true) return new ZExpression(Operator.NOT_HIGH_PREC, e);}
    throw new Error("Missing return statement in function");
  }

  final public ZExp SQLPrimaryExpression() throws ParseException {
  ZColRef col;
  Token t;
  String s, s2;
  int func_type = -1;
  ZExp e = null;
  int nparm;
  Vector parm = null;
    switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
    case K_NULL:
      jj_consume_token(K_NULL);
           {if (true) return new ZConstant("NULL", ZConstant.NULL);}
      break;
    default:
      jj_la1[96] = jj_gen;
      if (jj_2_18(2147483647)) {
        jj_consume_token(K_COUNT);
        jj_consume_token(115);
        switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
        case K_ALL:
          jj_consume_token(K_ALL);
          break;
        default:
          jj_la1[90] = jj_gen;
          ;
        }
        switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
        case 122:
          jj_consume_token(122);
          break;
        case K_AVG:
        case K_CASE:
        case K_COUNT:
        case K_DATE:
        case K_DAY:
        case K_EXISTS:
        case K_FALSE:
        case K_HOUR:
        case K_INTERVAL:
        case K_LEFT:
        case K_MAX:
        case K_MIN:
        case K_MOD:
        case K_MINUTE:
        case K_MONTH:
        case K_NOT:
        case K_NULL:
        case K_QUARTER:
        case K_RIGHT:
        case K_SECOND:
        case K_SUM:
        case K_TRUE:
        case K_WEEK:
        case K_YEAR:
        case S_NUMBER:
        case S_IDENTIFIER:
        case S_CHAR_LITERAL:
        case 115:
        case 138:
        case 139:
        case 143:
        case 144:
          e = SQLExpression();
          break;
        default:
          jj_la1[91] = jj_gen;
          jj_consume_token(-1);
          throw new ParseException();
        }
        jj_consume_token(117);
        parm = new Vector(1);
        if(e != null)parm.add(e);
     {if (true) return new ZExpression("COUNT", parm, false);}
      } else if (jj_2_19(2147483647)) {
        jj_consume_token(K_COUNT);
        jj_consume_token(115);
        jj_consume_token(K_DISTINCT);
        e = SQLExpression();
                               parm=new Vector(); parm.addElement(e);
        label_21:
        while (true) {
          switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
          case 116:
            ;
            break;
          default:
            jj_la1[92] = jj_gen;
            break label_21;
          }
          jj_consume_token(116);
          e = SQLExpression();
                               parm.addElement(e);
        }
        jj_consume_token(117);
     {if (true) return new ZExpression("COUNT", parm, true);}
      } else if (jj_2_20(2147483647)) {
        s = FunctionOrAggregateName();
        jj_consume_token(115);
        if (jj_2_16(2147483647)) {
          switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
          case K_ALL:
            jj_consume_token(K_ALL);
                                                 func_type = ZExpression.AGGR_ALL;
            break;
          case K_DISTINCT:
            jj_consume_token(K_DISTINCT);
                                                                                                    func_type = ZExpression.AGGR_DISTINCT;
            break;
          default:
            jj_la1[93] = jj_gen;
            jj_consume_token(-1);
            throw new ParseException();
          }
          e = SQLExpression();
        } else if (jj_2_17(2147483647)) {
                                           parm = new Vector(0);
        } else {
          switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
          case K_AVG:
          case K_CASE:
          case K_COUNT:
          case K_DATE:
          case K_DAY:
          case K_EXISTS:
          case K_FALSE:
          case K_HOUR:
          case K_INTERVAL:
          case K_LEFT:
          case K_MAX:
          case K_MIN:
          case K_MOD:
          case K_MINUTE:
          case K_MONTH:
          case K_NOT:
          case K_NULL:
          case K_QUARTER:
          case K_RIGHT:
          case K_SECOND:
          case K_SUM:
          case K_TRUE:
          case K_WEEK:
          case K_YEAR:
          case S_NUMBER:
          case S_IDENTIFIER:
          case S_CHAR_LITERAL:
          case 115:
          case 138:
          case 139:
          case 143:
          case 144:
            parm = SQLExpressionList();
            break;
          default:
            jj_la1[94] = jj_gen;
            jj_consume_token(-1);
            throw new ParseException();
          }
        }
        jj_consume_token(117);
            if( FunctionMgr.isAggregation(s) ){
                if(func_type == -1){
                        func_type = ZExpression.AGGR_ALL;
                        if(parm.size() != 1)
                                {if (true) throw new ParseException("Aggregate " + s + " should have 1 parameter");}
                        e = (ZExp)parm.get(0);
                }
                //add distinct judgement here 
                if( func_type == ZExpression.AGGR_DISTINCT && !FunctionMgr.aggrCanUseDistinct(s) )
                        {if (true) throw new ParseException("Cannot use DISTINCT on aggregation: " + s);}
                {if (true) return new ZExpression(s, e, func_type == ZExpression.AGGR_DISTINCT);}
            }

          else if( FunctionMgr.isFunction(s) ){
                if(func_type != -1)
                        {if (true) throw new ParseException("Function " + s + " should not have modifier ALL/DISTINCT");}
            nparm = FunctionMgr.getFuncParmN(s);
            if( nparm != ZExpression.VAR_PARAM && nparm != parm.size() ) {
              {if (true) throw new ParseException("Function " + s + " should have "
                 + nparm + " parameter(s)");}
            }
            {if (true) return new ZExpression(s, parm);}
          }

          else if ( FunctionMgr.isUdf(s) ){
                if(func_type != -1)
                        {if (true) throw new ParseException("UDF " + s + " should not have modifier ALL/DISTINCT");}
                nparm = FunctionMgr.getUdfParmN(s);
                if( parm.size() != nparm )
                        {if (true) throw new ParseException("Function " + s + " should have "
                         + nparm + " parameter(s)");}
            {if (true) return new ZExpression(s, parm, ZExpression.UDF);}
          }
          else {if (true) throw new ParseException("Undefined function/aggregation/UDF: " + s);}
      } else if (jj_2_21(2147483647)) {
        e = SQLInnerQuery(ZQuery.InnerQType.NORMAL);
                                                                            {if (true) return e;}
      } else if (jj_2_22(2147483647)) {
        jj_consume_token(115);
        e = SQLExpression();
        jj_consume_token(117);
                                                {if (true) return e;}
      } else {
        switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
        case S_IDENTIFIER:
          col = TableColumn();
                         {if (true) return col;}
          break;
        case S_NUMBER:
          t = jj_consume_token(S_NUMBER);
                    {if (true) return new ZConstant(t.toString(), ZConstant.NUMBER);}
          break;
        case S_CHAR_LITERAL:
          t = jj_consume_token(S_CHAR_LITERAL);
                          {if (true) return new ZConstant(t.toString(), ZConstant.STRING);}
          break;
        case K_FALSE:
        case K_TRUE:
          switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
          case K_TRUE:
            t = jj_consume_token(K_TRUE);
            break;
          case K_FALSE:
            t = jj_consume_token(K_FALSE);
            break;
          default:
            jj_la1[95] = jj_gen;
            jj_consume_token(-1);
            throw new ParseException();
          }
                                {if (true) return new ZConstant(t.toString(), ZConstant.BOOL);}
          break;
        case K_INTERVAL:
          e = Interval();
                   {if (true) return e;}
          break;
        default:
          jj_la1[97] = jj_gen;
          jj_consume_token(-1);
          throw new ParseException();
        }
      }
    }
    throw new Error("Missing return statement in function");
  }

  final public String FunctionOrAggregateName() throws ParseException {
  Token t;
  String s;
    switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
    case K_AVG:
    case K_DATE:
    case K_DAY:
    case K_HOUR:
    case K_LEFT:
    case K_MAX:
    case K_MIN:
    case K_MOD:
    case K_MINUTE:
    case K_MONTH:
    case K_QUARTER:
    case K_RIGHT:
    case K_SECOND:
    case K_SUM:
    case K_WEEK:
    case K_YEAR:
      switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
      case K_SUM:
        t = jj_consume_token(K_SUM);
        break;
      case K_AVG:
        t = jj_consume_token(K_AVG);
        break;
      case K_MAX:
        t = jj_consume_token(K_MAX);
        break;
      case K_MIN:
        t = jj_consume_token(K_MIN);
        break;
      case K_LEFT:
        t = jj_consume_token(K_LEFT);
        break;
      case K_RIGHT:
        t = jj_consume_token(K_RIGHT);
        break;
      case K_MOD:
        t = jj_consume_token(K_MOD);
        break;
      case K_DATE:
        t = jj_consume_token(K_DATE);
        break;
      case K_DAY:
        t = jj_consume_token(K_DAY);
        break;
      case K_HOUR:
        t = jj_consume_token(K_HOUR);
        break;
      case K_MINUTE:
        t = jj_consume_token(K_MINUTE);
        break;
      case K_MONTH:
        t = jj_consume_token(K_MONTH);
        break;
      case K_QUARTER:
        t = jj_consume_token(K_QUARTER);
        break;
      case K_SECOND:
        t = jj_consume_token(K_SECOND);
        break;
      case K_WEEK:
        t = jj_consume_token(K_WEEK);
        break;
      case K_YEAR:
        t = jj_consume_token(K_YEAR);
        break;
      default:
        jj_la1[98] = jj_gen;
        jj_consume_token(-1);
        throw new ParseException();
      }
          {if (true) return t.toString().toUpperCase();}
      break;
    case S_IDENTIFIER:
      s = identifier();
                           {if (true) return s;}
      break;
    default:
      jj_la1[99] = jj_gen;
      jj_consume_token(-1);
      throw new ParseException();
    }
    throw new Error("Missing return statement in function");
  }

  final public ZInterval Interval() throws ParseException {
        String type;
        ZExp exp;
    jj_consume_token(K_INTERVAL);
    exp = SQLExpression();
    type = IntervalType();
         {if (true) return new ZInterval(type, exp);}
    throw new Error("Missing return statement in function");
  }

  final public String IntervalType() throws ParseException {
        Token t;
    switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
    case K_SECOND:
      t = jj_consume_token(K_SECOND);
      break;
    case K_MINUTE:
      t = jj_consume_token(K_MINUTE);
      break;
    case K_HOUR:
      t = jj_consume_token(K_HOUR);
      break;
    case K_DAY:
      t = jj_consume_token(K_DAY);
      break;
    case K_WEEK:
      t = jj_consume_token(K_WEEK);
      break;
    case K_MONTH:
      t = jj_consume_token(K_MONTH);
      break;
    case K_QUARTER:
      t = jj_consume_token(K_QUARTER);
      break;
    case K_YEAR:
      t = jj_consume_token(K_YEAR);
      break;
    default:
      jj_la1[100] = jj_gen;
      jj_consume_token(-1);
      throw new ParseException();
    }
         {if (true) return t.toString();}
    throw new Error("Missing return statement in function");
  }

  final public ZQuery SQLInnerQuery(ZQuery.InnerQType type) throws ParseException {
  ZQuery q = new ZQuery();
  Vector<ZSelectItem> select = new Vector<ZSelectItem>(1);
  ZExp exp;
  ZFromClause from;
  ZExp where = null;
    jj_consume_token(115);
    jj_consume_token(K_SELECT);
    switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
    case 122:
      jj_consume_token(122);
           select.add(new ZSelectItem());
      break;
    case K_ALL:
    case K_AVG:
    case K_CASE:
    case K_COUNT:
    case K_DATE:
    case K_DAY:
    case K_DISTINCT:
    case K_EXISTS:
    case K_FALSE:
    case K_HOUR:
    case K_INTERVAL:
    case K_LEFT:
    case K_MAX:
    case K_MIN:
    case K_MOD:
    case K_MINUTE:
    case K_MONTH:
    case K_NOT:
    case K_NULL:
    case K_QUARTER:
    case K_RIGHT:
    case K_SECOND:
    case K_SUM:
    case K_TRUE:
    case K_WEEK:
    case K_YEAR:
    case S_NUMBER:
    case S_IDENTIFIER:
    case S_CHAR_LITERAL:
    case 115:
    case 138:
    case 139:
    case 143:
    case 144:
      switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
      case K_ALL:
      case K_DISTINCT:
        switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
        case K_ALL:
          jj_consume_token(K_ALL);
          break;
        case K_DISTINCT:
          jj_consume_token(K_DISTINCT);
                             q.distinct_ = true;
          break;
        default:
          jj_la1[101] = jj_gen;
          jj_consume_token(-1);
          throw new ParseException();
        }
        break;
      default:
        jj_la1[102] = jj_gen;
        ;
      }
      exp = SQLExpression();
                                  select.add(new ZSelectItem(exp, null));
      break;
    default:
      jj_la1[103] = jj_gen;
      jj_consume_token(-1);
      throw new ParseException();
    }
    from = FromClause();
    switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
    case K_WHERE:
      where = WhereClause();
      break;
    default:
      jj_la1[104] = jj_gen;
      ;
    }
    jj_consume_token(117);
        q.innerQType = type;
    q.setSelect(select);
    q.setFrom(from);
    q.setWhere(where);

    if(type == ZQuery.InnerQType.EXISTS && select.get(0).type != ZSelectItem.STAR)
        {if (true) throw new ParseException("EXISTS sub-clause must use * as select result: " + q.toString());}
    if(type != ZQuery.InnerQType.EXISTS && select.get(0).type != ZSelectItem.EXPR)
        {if (true) throw new ParseException(
        "IN/ALL/ANY sub-clause or other inner-query must use 1 expression or 1 colume as select result: "
         + q.toString());}
        {if (true) return q;}
    throw new Error("Missing return statement in function");
  }

  final private boolean jj_2_1(int xla) {
    jj_la = xla; jj_lastpos = jj_scanpos = token;
    try { return !jj_3_1(); }
    catch(LookaheadSuccess ls) { return true; }
    finally { jj_save(0, xla); }
  }

  final private boolean jj_2_2(int xla) {
    jj_la = xla; jj_lastpos = jj_scanpos = token;
    try { return !jj_3_2(); }
    catch(LookaheadSuccess ls) { return true; }
    finally { jj_save(1, xla); }
  }

  final private boolean jj_2_3(int xla) {
    jj_la = xla; jj_lastpos = jj_scanpos = token;
    try { return !jj_3_3(); }
    catch(LookaheadSuccess ls) { return true; }
    finally { jj_save(2, xla); }
  }

  final private boolean jj_2_4(int xla) {
    jj_la = xla; jj_lastpos = jj_scanpos = token;
    try { return !jj_3_4(); }
    catch(LookaheadSuccess ls) { return true; }
    finally { jj_save(3, xla); }
  }

  final private boolean jj_2_5(int xla) {
    jj_la = xla; jj_lastpos = jj_scanpos = token;
    try { return !jj_3_5(); }
    catch(LookaheadSuccess ls) { return true; }
    finally { jj_save(4, xla); }
  }

  final private boolean jj_2_6(int xla) {
    jj_la = xla; jj_lastpos = jj_scanpos = token;
    try { return !jj_3_6(); }
    catch(LookaheadSuccess ls) { return true; }
    finally { jj_save(5, xla); }
  }

  final private boolean jj_2_7(int xla) {
    jj_la = xla; jj_lastpos = jj_scanpos = token;
    try { return !jj_3_7(); }
    catch(LookaheadSuccess ls) { return true; }
    finally { jj_save(6, xla); }
  }

  final private boolean jj_2_8(int xla) {
    jj_la = xla; jj_lastpos = jj_scanpos = token;
    try { return !jj_3_8(); }
    catch(LookaheadSuccess ls) { return true; }
    finally { jj_save(7, xla); }
  }

  final private boolean jj_2_9(int xla) {
    jj_la = xla; jj_lastpos = jj_scanpos = token;
    try { return !jj_3_9(); }
    catch(LookaheadSuccess ls) { return true; }
    finally { jj_save(8, xla); }
  }

  final private boolean jj_2_10(int xla) {
    jj_la = xla; jj_lastpos = jj_scanpos = token;
    try { return !jj_3_10(); }
    catch(LookaheadSuccess ls) { return true; }
    finally { jj_save(9, xla); }
  }

  final private boolean jj_2_11(int xla) {
    jj_la = xla; jj_lastpos = jj_scanpos = token;
    try { return !jj_3_11(); }
    catch(LookaheadSuccess ls) { return true; }
    finally { jj_save(10, xla); }
  }

  final private boolean jj_2_12(int xla) {
    jj_la = xla; jj_lastpos = jj_scanpos = token;
    try { return !jj_3_12(); }
    catch(LookaheadSuccess ls) { return true; }
    finally { jj_save(11, xla); }
  }

  final private boolean jj_2_13(int xla) {
    jj_la = xla; jj_lastpos = jj_scanpos = token;
    try { return !jj_3_13(); }
    catch(LookaheadSuccess ls) { return true; }
    finally { jj_save(12, xla); }
  }

  final private boolean jj_2_14(int xla) {
    jj_la = xla; jj_lastpos = jj_scanpos = token;
    try { return !jj_3_14(); }
    catch(LookaheadSuccess ls) { return true; }
    finally { jj_save(13, xla); }
  }

  final private boolean jj_2_15(int xla) {
    jj_la = xla; jj_lastpos = jj_scanpos = token;
    try { return !jj_3_15(); }
    catch(LookaheadSuccess ls) { return true; }
    finally { jj_save(14, xla); }
  }

  final private boolean jj_2_16(int xla) {
    jj_la = xla; jj_lastpos = jj_scanpos = token;
    try { return !jj_3_16(); }
    catch(LookaheadSuccess ls) { return true; }
    finally { jj_save(15, xla); }
  }

  final private boolean jj_2_17(int xla) {
    jj_la = xla; jj_lastpos = jj_scanpos = token;
    try { return !jj_3_17(); }
    catch(LookaheadSuccess ls) { return true; }
    finally { jj_save(16, xla); }
  }

  final private boolean jj_2_18(int xla) {
    jj_la = xla; jj_lastpos = jj_scanpos = token;
    try { return !jj_3_18(); }
    catch(LookaheadSuccess ls) { return true; }
    finally { jj_save(17, xla); }
  }

  final private boolean jj_2_19(int xla) {
    jj_la = xla; jj_lastpos = jj_scanpos = token;
    try { return !jj_3_19(); }
    catch(LookaheadSuccess ls) { return true; }
    finally { jj_save(18, xla); }
  }

  final private boolean jj_2_20(int xla) {
    jj_la = xla; jj_lastpos = jj_scanpos = token;
    try { return !jj_3_20(); }
    catch(LookaheadSuccess ls) { return true; }
    finally { jj_save(19, xla); }
  }

  final private boolean jj_2_21(int xla) {
    jj_la = xla; jj_lastpos = jj_scanpos = token;
    try { return !jj_3_21(); }
    catch(LookaheadSuccess ls) { return true; }
    finally { jj_save(20, xla); }
  }

  final private boolean jj_2_22(int xla) {
    jj_la = xla; jj_lastpos = jj_scanpos = token;
    try { return !jj_3_22(); }
    catch(LookaheadSuccess ls) { return true; }
    finally { jj_save(21, xla); }
  }

  final private boolean jj_3R_136() {
    Token xsp;
    xsp = jj_scanpos;
    if (jj_scan_token(138)) {
    jj_scanpos = xsp;
    if (jj_3R_142()) {
    jj_scanpos = xsp;
    if (jj_3R_143()) return true;
    }
    }
    return false;
  }

  final private boolean jj_3R_67() {
    Token xsp;
    xsp = jj_scanpos;
    if (jj_scan_token(65)) {
    jj_scanpos = xsp;
    if (jj_scan_token(124)) return true;
    }
    if (jj_3R_66()) return true;
    return false;
  }

  final private boolean jj_3R_119() {
    Token xsp;
    xsp = jj_scanpos;
    if (jj_3R_136()) jj_scanpos = xsp;
    if (jj_3R_137()) return true;
    return false;
  }

  final private boolean jj_3R_53() {
    if (jj_3R_66()) return true;
    Token xsp;
    while (true) {
      xsp = jj_scanpos;
      if (jj_3R_67()) { jj_scanpos = xsp; break; }
    }
    return false;
  }

  final private boolean jj_3R_135() {
    if (jj_scan_token(K_DESC)) return true;
    return false;
  }

  final private boolean jj_3R_116() {
    if (jj_scan_token(K_DESC)) return true;
    return false;
  }

  final private boolean jj_3R_122() {
    if (jj_scan_token(140)) return true;
    return false;
  }

  final private boolean jj_3R_117() {
    Token xsp;
    xsp = jj_scanpos;
    if (jj_scan_token(9)) {
    jj_scanpos = xsp;
    if (jj_3R_135()) return true;
    }
    return false;
  }

  final private boolean jj_3R_103() {
    if (jj_scan_token(116)) return true;
    if (jj_3R_53()) return true;
    Token xsp;
    xsp = jj_scanpos;
    if (jj_3R_117()) jj_scanpos = xsp;
    return false;
  }

  final private boolean jj_3R_102() {
    Token xsp;
    xsp = jj_scanpos;
    if (jj_scan_token(9)) {
    jj_scanpos = xsp;
    if (jj_3R_116()) return true;
    }
    return false;
  }

  final private boolean jj_3_1() {
    if (jj_scan_token(0)) return true;
    return false;
  }

  final private boolean jj_3R_85() {
    if (jj_scan_token(K_ORDER)) return true;
    if (jj_scan_token(K_BY)) return true;
    if (jj_3R_53()) return true;
    Token xsp;
    xsp = jj_scanpos;
    if (jj_3R_102()) jj_scanpos = xsp;
    while (true) {
      xsp = jj_scanpos;
      if (jj_3R_103()) { jj_scanpos = xsp; break; }
    }
    return false;
  }

  final private boolean jj_3R_120() {
    if (jj_scan_token(142)) return true;
    if (jj_3R_119()) return true;
    return false;
  }

  final private boolean jj_3R_105() {
    if (jj_3R_119()) return true;
    Token xsp;
    while (true) {
      xsp = jj_scanpos;
      if (jj_3R_120()) { jj_scanpos = xsp; break; }
    }
    return false;
  }

  final private boolean jj_3R_141() {
    if (jj_scan_token(116)) return true;
    if (jj_3R_147()) return true;
    return false;
  }

  final private boolean jj_3R_133() {
    if (jj_3R_53()) return true;
    Token xsp;
    while (true) {
      xsp = jj_scanpos;
      if (jj_3R_141()) { jj_scanpos = xsp; break; }
    }
    return false;
  }

  final private boolean jj_3R_108() {
    if (jj_scan_token(139)) return true;
    return false;
  }

  final private boolean jj_3R_134() {
    if (jj_scan_token(K_HAVING)) return true;
    if (jj_3R_53()) return true;
    return false;
  }

  final private boolean jj_3R_121() {
    if (jj_scan_token(122)) return true;
    return false;
  }

  final private boolean jj_3R_22() {
    if (jj_scan_token(S_IDENTIFIER)) return true;
    return false;
  }

  final private boolean jj_3R_115() {
    if (jj_scan_token(K_HAVING)) return true;
    if (jj_3R_53()) return true;
    return false;
  }

  final private boolean jj_3R_106() {
    Token xsp;
    xsp = jj_scanpos;
    if (jj_3R_121()) {
    jj_scanpos = xsp;
    if (jj_3R_122()) {
    jj_scanpos = xsp;
    if (jj_3R_123()) {
    jj_scanpos = xsp;
    if (jj_3R_124()) {
    jj_scanpos = xsp;
    if (jj_3R_125()) return true;
    }
    }
    }
    }
    if (jj_3R_105()) return true;
    return false;
  }

  final private boolean jj_3R_114() {
    if (jj_scan_token(K_GROUP)) return true;
    if (jj_scan_token(K_BY)) return true;
    if (jj_3R_133()) return true;
    Token xsp;
    xsp = jj_scanpos;
    if (jj_3R_134()) jj_scanpos = xsp;
    return false;
  }

  final private boolean jj_3R_101() {
    Token xsp;
    xsp = jj_scanpos;
    if (jj_3R_114()) {
    jj_scanpos = xsp;
    if (jj_3R_115()) return true;
    }
    return false;
  }

  final private boolean jj_3R_89() {
    if (jj_3R_105()) return true;
    Token xsp;
    while (true) {
      xsp = jj_scanpos;
      if (jj_3R_106()) { jj_scanpos = xsp; break; }
    }
    return false;
  }

  final private boolean jj_3R_130() {
    if (jj_scan_token(K_RIGHT)) return true;
    return false;
  }

  final private boolean jj_3R_92() {
    if (jj_scan_token(137)) return true;
    return false;
  }

  final private boolean jj_3R_100() {
    if (jj_scan_token(K_WHERE)) return true;
    if (jj_3R_53()) return true;
    return false;
  }

  final private boolean jj_3R_107() {
    if (jj_scan_token(138)) return true;
    return false;
  }

  final private boolean jj_3R_90() {
    Token xsp;
    xsp = jj_scanpos;
    if (jj_3R_107()) {
    jj_scanpos = xsp;
    if (jj_3R_108()) return true;
    }
    if (jj_3R_89()) return true;
    return false;
  }

  final private boolean jj_3R_33() {
    Token xsp;
    xsp = jj_scanpos;
    if (jj_scan_token(8)) jj_scanpos = xsp;
    if (jj_3R_22()) return true;
    return false;
  }

  final private boolean jj_3R_32() {
    if (jj_scan_token(115)) return true;
    if (jj_3R_57()) return true;
    if (jj_scan_token(117)) return true;
    return false;
  }

  final private boolean jj_3R_31() {
    if (jj_3R_56()) return true;
    return false;
  }

  final private boolean jj_3R_76() {
    if (jj_3R_89()) return true;
    Token xsp;
    while (true) {
      xsp = jj_scanpos;
      if (jj_3R_90()) { jj_scanpos = xsp; break; }
    }
    return false;
  }

  final private boolean jj_3R_24() {
    Token xsp;
    xsp = jj_scanpos;
    if (jj_3R_31()) {
    jj_scanpos = xsp;
    if (jj_3R_32()) return true;
    }
    xsp = jj_scanpos;
    if (jj_3R_33()) jj_scanpos = xsp;
    return false;
  }

  final private boolean jj_3R_91() {
    if (jj_scan_token(136)) return true;
    return false;
  }

  final private boolean jj_3R_77() {
    Token xsp;
    xsp = jj_scanpos;
    if (jj_3R_91()) {
    jj_scanpos = xsp;
    if (jj_3R_92()) return true;
    }
    if (jj_3R_76()) return true;
    return false;
  }

  final private boolean jj_3R_131() {
    if (jj_scan_token(K_ON)) return true;
    if (jj_3R_53()) return true;
    return false;
  }

  final private boolean jj_3R_129() {
    if (jj_scan_token(K_LEFT)) return true;
    return false;
  }

  final private boolean jj_3R_112() {
    if (jj_3R_24()) return true;
    Token xsp;
    xsp = jj_scanpos;
    if (jj_3R_129()) {
    jj_scanpos = xsp;
    if (jj_3R_130()) return true;
    }
    xsp = jj_scanpos;
    if (jj_scan_token(67)) jj_scanpos = xsp;
    if (jj_scan_token(K_JOIN)) return true;
    if (jj_3R_24()) return true;
    xsp = jj_scanpos;
    if (jj_3R_131()) jj_scanpos = xsp;
    return false;
  }

  final private boolean jj_3R_64() {
    if (jj_3R_76()) return true;
    Token xsp;
    while (true) {
      xsp = jj_scanpos;
      if (jj_3R_77()) { jj_scanpos = xsp; break; }
    }
    return false;
  }

  final private boolean jj_3_6() {
    if (jj_3R_24()) return true;
    Token xsp;
    xsp = jj_scanpos;
    if (jj_scan_token(47)) {
    jj_scanpos = xsp;
    if (jj_scan_token(73)) return true;
    }
    return false;
  }

  final private boolean jj_3R_132() {
    if (jj_scan_token(116)) return true;
    if (jj_3R_24()) return true;
    return false;
  }

  final private boolean jj_3R_65() {
    if (jj_scan_token(135)) return true;
    if (jj_3R_64()) return true;
    return false;
  }

  final private boolean jj_3R_99() {
    if (jj_3R_113()) return true;
    return false;
  }

  final private boolean jj_3R_113() {
    if (jj_3R_24()) return true;
    Token xsp;
    while (true) {
      xsp = jj_scanpos;
      if (jj_3R_132()) { jj_scanpos = xsp; break; }
    }
    return false;
  }

  final private boolean jj_3R_43() {
    if (jj_3R_64()) return true;
    Token xsp;
    while (true) {
      xsp = jj_scanpos;
      if (jj_3R_65()) { jj_scanpos = xsp; break; }
    }
    return false;
  }

  final private boolean jj_3R_98() {
    if (jj_3R_112()) return true;
    return false;
  }

  final private boolean jj_3R_82() {
    if (jj_scan_token(K_FROM)) return true;
    Token xsp;
    xsp = jj_scanpos;
    if (jj_3R_98()) {
    jj_scanpos = xsp;
    if (jj_3R_99()) return true;
    }
    return false;
  }

  final private boolean jj_3R_44() {
    if (jj_scan_token(134)) return true;
    if (jj_3R_43()) return true;
    return false;
  }

  final private boolean jj_3R_27() {
    if (jj_3R_43()) return true;
    Token xsp;
    while (true) {
      xsp = jj_scanpos;
      if (jj_3R_44()) { jj_scanpos = xsp; break; }
    }
    return false;
  }

  final private boolean jj_3R_111() {
    if (jj_3R_53()) return true;
    Token xsp;
    xsp = jj_scanpos;
    if (jj_3R_128()) jj_scanpos = xsp;
    return false;
  }

  final private boolean jj_3_5() {
    if (jj_3R_22()) return true;
    if (jj_scan_token(123)) return true;
    return false;
  }

  final private boolean jj_3R_128() {
    Token xsp;
    xsp = jj_scanpos;
    if (jj_scan_token(8)) jj_scanpos = xsp;
    if (jj_3R_22()) return true;
    return false;
  }

  final private boolean jj_3R_96() {
    Token xsp;
    xsp = jj_scanpos;
    if (jj_3R_110()) {
    jj_scanpos = xsp;
    if (jj_3_5()) {
    jj_scanpos = xsp;
    if (jj_3R_111()) return true;
    }
    }
    return false;
  }

  final private boolean jj_3R_110() {
    if (jj_scan_token(122)) return true;
    return false;
  }

  final private boolean jj_3R_75() {
    if (jj_scan_token(K_NOT)) return true;
    return false;
  }

  final private boolean jj_3R_63() {
    if (jj_3R_27()) return true;
    Token xsp;
    xsp = jj_scanpos;
    if (jj_3R_75()) jj_scanpos = xsp;
    xsp = jj_scanpos;
    if (jj_scan_token(48)) {
    jj_scanpos = xsp;
    if (jj_scan_token(74)) return true;
    }
    if (jj_3R_27()) return true;
    return false;
  }

  final private boolean jj_3R_62() {
    if (jj_3R_27()) return true;
    Token xsp;
    xsp = jj_scanpos;
    if (jj_3R_74()) jj_scanpos = xsp;
    if (jj_scan_token(K_LIKE)) return true;
    if (jj_3R_27()) return true;
    return false;
  }

  final private boolean jj_3R_74() {
    if (jj_scan_token(K_NOT)) return true;
    return false;
  }

  final private boolean jj_3R_73() {
    if (jj_scan_token(K_NOT)) return true;
    return false;
  }

  final private boolean jj_3R_97() {
    if (jj_scan_token(116)) return true;
    if (jj_3R_96()) return true;
    return false;
  }

  final private boolean jj_3R_81() {
    if (jj_3R_96()) return true;
    Token xsp;
    while (true) {
      xsp = jj_scanpos;
      if (jj_3R_97()) { jj_scanpos = xsp; break; }
    }
    return false;
  }

  final private boolean jj_3R_174() {
    if (jj_scan_token(116)) return true;
    if (jj_3R_53()) return true;
    return false;
  }

  final private boolean jj_3R_172() {
    if (jj_3R_53()) return true;
    Token xsp;
    while (true) {
      xsp = jj_scanpos;
      if (jj_3R_174()) { jj_scanpos = xsp; break; }
    }
    return false;
  }

  final private boolean jj_3R_95() {
    if (jj_scan_token(K_DISTINCT)) return true;
    return false;
  }

  final private boolean jj_3_4() {
    Token xsp;
    xsp = jj_scanpos;
    if (jj_3R_23()) {
    jj_scanpos = xsp;
    if (jj_scan_token(37)) return true;
    }
    return false;
  }

  final private boolean jj_3R_23() {
    if (jj_scan_token(K_GROUP)) return true;
    if (jj_scan_token(K_BY)) return true;
    return false;
  }

  final private boolean jj_3R_72() {
    if (jj_scan_token(K_NOT)) return true;
    return false;
  }

  final private boolean jj_3R_118() {
    if (jj_scan_token(K_DISTINCT)) return true;
    return false;
  }

  final private boolean jj_3R_71() {
    Token xsp;
    xsp = jj_scanpos;
    if (jj_scan_token(7)) {
    jj_scanpos = xsp;
    if (jj_scan_token(83)) return true;
    }
    return false;
  }

  final private boolean jj_3R_80() {
    Token xsp;
    xsp = jj_scanpos;
    if (jj_scan_token(5)) {
    jj_scanpos = xsp;
    if (jj_3R_95()) return true;
    }
    return false;
  }

  final private boolean jj_3R_84() {
    if (jj_3R_101()) return true;
    return false;
  }

  final private boolean jj_3R_83() {
    if (jj_3R_100()) return true;
    return false;
  }

  final private boolean jj_3R_61() {
    if (jj_3R_27()) return true;
    if (jj_scan_token(K_IS)) return true;
    Token xsp;
    xsp = jj_scanpos;
    if (jj_3R_73()) jj_scanpos = xsp;
    if (jj_scan_token(K_NULL)) return true;
    return false;
  }

  final private boolean jj_3R_88() {
    if (jj_3R_100()) return true;
    return false;
  }

  final private boolean jj_3R_104() {
    Token xsp;
    xsp = jj_scanpos;
    if (jj_scan_token(5)) {
    jj_scanpos = xsp;
    if (jj_3R_118()) return true;
    }
    return false;
  }

  final private boolean jj_3R_87() {
    Token xsp;
    xsp = jj_scanpos;
    if (jj_3R_104()) jj_scanpos = xsp;
    if (jj_3R_53()) return true;
    return false;
  }

  final private boolean jj_3R_68() {
    if (jj_scan_token(K_SELECT)) return true;
    Token xsp;
    xsp = jj_scanpos;
    if (jj_3R_80()) jj_scanpos = xsp;
    if (jj_3R_81()) return true;
    if (jj_3R_82()) return true;
    xsp = jj_scanpos;
    if (jj_3R_83()) jj_scanpos = xsp;
    xsp = jj_scanpos;
    if (jj_3R_84()) jj_scanpos = xsp;
    return false;
  }

  final private boolean jj_3R_86() {
    if (jj_scan_token(122)) return true;
    return false;
  }

  final private boolean jj_3R_60() {
    if (jj_3R_27()) return true;
    if (jj_3R_28()) return true;
    if (jj_3R_27()) return true;
    return false;
  }

  final private boolean jj_3R_35() {
    if (jj_scan_token(115)) return true;
    if (jj_scan_token(K_SELECT)) return true;
    Token xsp;
    xsp = jj_scanpos;
    if (jj_3R_86()) {
    jj_scanpos = xsp;
    if (jj_3R_87()) return true;
    }
    if (jj_3R_82()) return true;
    xsp = jj_scanpos;
    if (jj_3R_88()) jj_scanpos = xsp;
    if (jj_scan_token(117)) return true;
    return false;
  }

  final private boolean jj_3R_59() {
    if (jj_3R_27()) return true;
    Token xsp;
    xsp = jj_scanpos;
    if (jj_3R_72()) jj_scanpos = xsp;
    if (jj_scan_token(K_IN)) return true;
    if (jj_3R_35()) return true;
    return false;
  }

  final private boolean jj_3R_69() {
    if (jj_3R_85()) return true;
    return false;
  }

  final private boolean jj_3R_57() {
    if (jj_3R_68()) return true;
    Token xsp;
    xsp = jj_scanpos;
    if (jj_3R_69()) jj_scanpos = xsp;
    return false;
  }

  final private boolean jj_3R_173() {
    Token xsp;
    xsp = jj_scanpos;
    if (jj_scan_token(78)) {
    jj_scanpos = xsp;
    if (jj_scan_token(54)) {
    jj_scanpos = xsp;
    if (jj_scan_token(38)) {
    jj_scanpos = xsp;
    if (jj_scan_token(22)) {
    jj_scanpos = xsp;
    if (jj_scan_token(95)) {
    jj_scanpos = xsp;
    if (jj_scan_token(56)) {
    jj_scanpos = xsp;
    if (jj_scan_token(69)) {
    jj_scanpos = xsp;
    if (jj_scan_token(102)) return true;
    }
    }
    }
    }
    }
    }
    }
    return false;
  }

  final private boolean jj_3R_56() {
    if (jj_3R_22()) return true;
    return false;
  }

  final private boolean jj_3R_70() {
    if (jj_scan_token(K_ALL)) return true;
    return false;
  }

  final private boolean jj_3R_58() {
    if (jj_3R_27()) return true;
    if (jj_3R_28()) return true;
    Token xsp;
    xsp = jj_scanpos;
    if (jj_3R_70()) {
    jj_scanpos = xsp;
    if (jj_3R_71()) return true;
    }
    if (jj_3R_35()) return true;
    return false;
  }

  final private boolean jj_3R_163() {
    if (jj_3R_22()) return true;
    return false;
  }

  final private boolean jj_3R_169() {
    if (jj_scan_token(K_INTERVAL)) return true;
    if (jj_3R_53()) return true;
    if (jj_3R_173()) return true;
    return false;
  }

  final private boolean jj_3_3() {
    if (jj_3R_22()) return true;
    if (jj_scan_token(121)) return true;
    if (jj_3R_22()) return true;
    return false;
  }

  final private boolean jj_3_15() {
    if (jj_3R_27()) return true;
    Token xsp;
    xsp = jj_scanpos;
    if (jj_scan_token(58)) jj_scanpos = xsp;
    xsp = jj_scanpos;
    if (jj_scan_token(75)) {
    jj_scanpos = xsp;
    if (jj_scan_token(74)) return true;
    }
    return false;
  }

  final private boolean jj_3R_147() {
    Token xsp;
    xsp = jj_scanpos;
    if (jj_3_3()) {
    jj_scanpos = xsp;
    if (jj_3R_163()) return true;
    }
    return false;
  }

  final private boolean jj_3_14() {
    if (jj_3R_27()) return true;
    Token xsp;
    xsp = jj_scanpos;
    if (jj_scan_token(58)) jj_scanpos = xsp;
    if (jj_scan_token(K_LIKE)) return true;
    return false;
  }

  final private boolean jj_3_13() {
    if (jj_3R_27()) return true;
    if (jj_scan_token(K_IS)) return true;
    return false;
  }

  final private boolean jj_3_11() {
    if (jj_3R_27()) return true;
    Token xsp;
    xsp = jj_scanpos;
    if (jj_scan_token(58)) jj_scanpos = xsp;
    if (jj_scan_token(K_IN)) return true;
    return false;
  }

  final private boolean jj_3_10() {
    if (jj_3R_27()) return true;
    if (jj_3R_28()) return true;
    Token xsp;
    xsp = jj_scanpos;
    if (jj_scan_token(5)) {
    jj_scanpos = xsp;
    if (jj_scan_token(7)) {
    jj_scanpos = xsp;
    if (jj_scan_token(83)) return true;
    }
    }
    return false;
  }

  final private boolean jj_3_12() {
    if (jj_3R_27()) return true;
    if (jj_3R_28()) return true;
    return false;
  }

  final private boolean jj_3R_55() {
    if (jj_3R_22()) return true;
    return false;
  }

  final private boolean jj_3R_171() {
    if (jj_scan_token(K_DISTINCT)) return true;
    return false;
  }

  final private boolean jj_3R_42() {
    if (jj_3R_27()) return true;
    return false;
  }

  final private boolean jj_3R_41() {
    if (jj_3R_63()) return true;
    return false;
  }

  final private boolean jj_3R_40() {
    if (jj_3R_62()) return true;
    return false;
  }

  final private boolean jj_3R_39() {
    if (jj_3R_61()) return true;
    return false;
  }

  final private boolean jj_3R_37() {
    if (jj_3R_59()) return true;
    return false;
  }

  final private boolean jj_3R_36() {
    if (jj_3R_58()) return true;
    return false;
  }

  final private boolean jj_3R_26() {
    Token xsp;
    xsp = jj_scanpos;
    if (jj_3R_36()) {
    jj_scanpos = xsp;
    if (jj_3R_37()) {
    jj_scanpos = xsp;
    if (jj_3R_38()) {
    jj_scanpos = xsp;
    if (jj_3R_39()) {
    jj_scanpos = xsp;
    if (jj_3R_40()) {
    jj_scanpos = xsp;
    if (jj_3R_41()) {
    jj_scanpos = xsp;
    if (jj_3R_42()) return true;
    }
    }
    }
    }
    }
    }
    return false;
  }

  final private boolean jj_3R_38() {
    if (jj_3R_60()) return true;
    return false;
  }

  final private boolean jj_3R_54() {
    Token xsp;
    xsp = jj_scanpos;
    if (jj_scan_token(85)) {
    jj_scanpos = xsp;
    if (jj_scan_token(10)) {
    jj_scanpos = xsp;
    if (jj_scan_token(50)) {
    jj_scanpos = xsp;
    if (jj_scan_token(51)) {
    jj_scanpos = xsp;
    if (jj_scan_token(47)) {
    jj_scanpos = xsp;
    if (jj_scan_token(73)) {
    jj_scanpos = xsp;
    if (jj_scan_token(52)) {
    jj_scanpos = xsp;
    if (jj_scan_token(21)) {
    jj_scanpos = xsp;
    if (jj_scan_token(22)) {
    jj_scanpos = xsp;
    if (jj_scan_token(38)) {
    jj_scanpos = xsp;
    if (jj_scan_token(54)) {
    jj_scanpos = xsp;
    if (jj_scan_token(56)) {
    jj_scanpos = xsp;
    if (jj_scan_token(69)) {
    jj_scanpos = xsp;
    if (jj_scan_token(78)) {
    jj_scanpos = xsp;
    if (jj_scan_token(95)) {
    jj_scanpos = xsp;
    if (jj_scan_token(102)) return true;
    }
    }
    }
    }
    }
    }
    }
    }
    }
    }
    }
    }
    }
    }
    }
    return false;
  }

  final private boolean jj_3R_30() {
    Token xsp;
    xsp = jj_scanpos;
    if (jj_3R_54()) {
    jj_scanpos = xsp;
    if (jj_3R_55()) return true;
    }
    return false;
  }

  final private boolean jj_3R_52() {
    if (jj_scan_token(132)) return true;
    return false;
  }

  final private boolean jj_3R_51() {
    if (jj_scan_token(131)) return true;
    return false;
  }

  final private boolean jj_3R_50() {
    if (jj_scan_token(130)) return true;
    return false;
  }

  final private boolean jj_3R_49() {
    if (jj_scan_token(129)) return true;
    return false;
  }

  final private boolean jj_3R_48() {
    if (jj_scan_token(128)) return true;
    return false;
  }

  final private boolean jj_3_21() {
    if (jj_scan_token(115)) return true;
    if (jj_scan_token(K_SELECT)) return true;
    return false;
  }

  final private boolean jj_3R_47() {
    if (jj_scan_token(127)) return true;
    return false;
  }

  final private boolean jj_3R_46() {
    if (jj_scan_token(126)) return true;
    return false;
  }

  final private boolean jj_3R_45() {
    if (jj_scan_token(120)) return true;
    return false;
  }

  final private boolean jj_3R_28() {
    Token xsp;
    xsp = jj_scanpos;
    if (jj_3R_45()) {
    jj_scanpos = xsp;
    if (jj_3R_46()) {
    jj_scanpos = xsp;
    if (jj_3R_47()) {
    jj_scanpos = xsp;
    if (jj_3R_48()) {
    jj_scanpos = xsp;
    if (jj_3R_49()) {
    jj_scanpos = xsp;
    if (jj_3R_50()) {
    jj_scanpos = xsp;
    if (jj_3R_51()) {
    jj_scanpos = xsp;
    if (jj_3R_52()) return true;
    }
    }
    }
    }
    }
    }
    }
    return false;
  }

  final private boolean jj_3_22() {
    if (jj_scan_token(115)) return true;
    return false;
  }

  final private boolean jj_3R_158() {
    if (jj_3R_169()) return true;
    return false;
  }

  final private boolean jj_3R_157() {
    Token xsp;
    xsp = jj_scanpos;
    if (jj_scan_token(89)) {
    jj_scanpos = xsp;
    if (jj_scan_token(34)) return true;
    }
    return false;
  }

  final private boolean jj_3R_156() {
    if (jj_scan_token(S_CHAR_LITERAL)) return true;
    return false;
  }

  final private boolean jj_3R_155() {
    if (jj_scan_token(S_NUMBER)) return true;
    return false;
  }

  final private boolean jj_3R_154() {
    if (jj_3R_147()) return true;
    return false;
  }

  final private boolean jj_3R_162() {
    if (jj_scan_token(K_NOT)) return true;
    return false;
  }

  final private boolean jj_3R_153() {
    if (jj_scan_token(115)) return true;
    if (jj_3R_53()) return true;
    if (jj_scan_token(117)) return true;
    return false;
  }

  final private boolean jj_3R_152() {
    if (jj_3R_35()) return true;
    return false;
  }

  final private boolean jj_3R_159() {
    if (jj_3R_26()) return true;
    return false;
  }

  final private boolean jj_3R_146() {
    if (jj_3R_26()) return true;
    Token xsp;
    xsp = jj_scanpos;
    if (jj_3R_162()) jj_scanpos = xsp;
    if (jj_scan_token(K_BETWEEN)) return true;
    if (jj_3R_26()) return true;
    if (jj_scan_token(K_AND)) return true;
    if (jj_3R_26()) return true;
    return false;
  }

  final private boolean jj_3R_161() {
    if (jj_scan_token(K_ELSE)) return true;
    if (jj_3R_26()) return true;
    return false;
  }

  final private boolean jj_3R_170() {
    if (jj_scan_token(K_ALL)) return true;
    return false;
  }

  final private boolean jj_3_17() {
    if (jj_scan_token(117)) return true;
    return false;
  }

  final private boolean jj_3_2() {
    Token xsp;
    if (jj_scan_token(115)) return true;
    while (true) {
      xsp = jj_scanpos;
      if (jj_scan_token(115)) { jj_scanpos = xsp; break; }
    }
    if (jj_scan_token(K_SELECT)) return true;
    return false;
  }

  final private boolean jj_3R_160() {
    if (jj_scan_token(K_WHEN)) return true;
    if (jj_3R_26()) return true;
    if (jj_scan_token(K_THEN)) return true;
    if (jj_3R_26()) return true;
    return false;
  }

  final private boolean jj_3R_145() {
    if (jj_scan_token(K_CASE)) return true;
    Token xsp;
    xsp = jj_scanpos;
    if (jj_3R_159()) jj_scanpos = xsp;
    if (jj_3R_160()) return true;
    while (true) {
      xsp = jj_scanpos;
      if (jj_3R_160()) { jj_scanpos = xsp; break; }
    }
    xsp = jj_scanpos;
    if (jj_3R_161()) jj_scanpos = xsp;
    if (jj_scan_token(K_END)) return true;
    return false;
  }

  final private boolean jj_3_16() {
    Token xsp;
    xsp = jj_scanpos;
    if (jj_scan_token(5)) {
    jj_scanpos = xsp;
    if (jj_scan_token(25)) return true;
    }
    return false;
  }

  final private boolean jj_3_9() {
    if (jj_3R_26()) return true;
    Token xsp;
    xsp = jj_scanpos;
    if (jj_scan_token(58)) jj_scanpos = xsp;
    if (jj_scan_token(K_BETWEEN)) return true;
    return false;
  }

  final private boolean jj_3R_168() {
    if (jj_3R_172()) return true;
    return false;
  }

  final private boolean jj_3_8() {
    if (jj_scan_token(K_CASE)) return true;
    return false;
  }

  final private boolean jj_3R_167() {
    return false;
  }

  final private boolean jj_3R_140() {
    if (jj_3R_26()) return true;
    return false;
  }

  final private boolean jj_3R_166() {
    Token xsp;
    xsp = jj_scanpos;
    if (jj_3R_170()) {
    jj_scanpos = xsp;
    if (jj_3R_171()) return true;
    }
    if (jj_3R_53()) return true;
    return false;
  }

  final private boolean jj_3R_139() {
    if (jj_3R_146()) return true;
    return false;
  }

  final private boolean jj_3R_127() {
    Token xsp;
    xsp = jj_scanpos;
    if (jj_3R_138()) {
    jj_scanpos = xsp;
    if (jj_3R_139()) {
    jj_scanpos = xsp;
    if (jj_3R_140()) return true;
    }
    }
    return false;
  }

  final private boolean jj_3R_138() {
    if (jj_3R_145()) return true;
    return false;
  }

  final private boolean jj_3R_125() {
    if (jj_scan_token(K_MOD)) return true;
    return false;
  }

  final private boolean jj_3R_29() {
    if (jj_3R_53()) return true;
    return false;
  }

  final private boolean jj_3R_164() {
    if (jj_3R_53()) return true;
    return false;
  }

  final private boolean jj_3R_34() {
    if (jj_scan_token(K_NOT)) return true;
    return false;
  }

  final private boolean jj_3_20() {
    if (jj_3R_30()) return true;
    if (jj_scan_token(115)) return true;
    return false;
  }

  final private boolean jj_3R_25() {
    Token xsp;
    xsp = jj_scanpos;
    if (jj_3R_34()) jj_scanpos = xsp;
    if (jj_scan_token(K_EXISTS)) return true;
    if (jj_3R_35()) return true;
    return false;
  }

  final private boolean jj_3_19() {
    if (jj_scan_token(K_COUNT)) return true;
    if (jj_scan_token(115)) return true;
    if (jj_scan_token(K_DISTINCT)) return true;
    return false;
  }

  final private boolean jj_3R_151() {
    if (jj_3R_30()) return true;
    if (jj_scan_token(115)) return true;
    Token xsp;
    xsp = jj_scanpos;
    if (jj_3R_166()) {
    jj_scanpos = xsp;
    if (jj_3R_167()) {
    jj_scanpos = xsp;
    if (jj_3R_168()) return true;
    }
    }
    if (jj_scan_token(117)) return true;
    return false;
  }

  final private boolean jj_3R_165() {
    if (jj_scan_token(116)) return true;
    if (jj_3R_53()) return true;
    return false;
  }

  final private boolean jj_3R_126() {
    if (jj_scan_token(K_NOT)) return true;
    return false;
  }

  final private boolean jj_3_18() {
    if (jj_scan_token(K_COUNT)) return true;
    if (jj_scan_token(115)) return true;
    Token xsp;
    xsp = jj_scanpos;
    if (jj_scan_token(5)) jj_scanpos = xsp;
    xsp = jj_scanpos;
    if (jj_scan_token(122)) {
    jj_scanpos = xsp;
    if (jj_3R_29()) return true;
    }
    if (jj_scan_token(117)) return true;
    return false;
  }

  final private boolean jj_3R_109() {
    Token xsp;
    xsp = jj_scanpos;
    if (jj_3R_126()) jj_scanpos = xsp;
    if (jj_3R_127()) return true;
    return false;
  }

  final private boolean jj_3_7() {
    if (jj_3R_25()) return true;
    return false;
  }

  final private boolean jj_3R_93() {
    Token xsp;
    xsp = jj_scanpos;
    if (jj_3_7()) {
    jj_scanpos = xsp;
    if (jj_3R_109()) return true;
    }
    return false;
  }

  final private boolean jj_3R_150() {
    if (jj_scan_token(K_COUNT)) return true;
    if (jj_scan_token(115)) return true;
    if (jj_scan_token(K_DISTINCT)) return true;
    if (jj_3R_53()) return true;
    Token xsp;
    while (true) {
      xsp = jj_scanpos;
      if (jj_3R_165()) { jj_scanpos = xsp; break; }
    }
    if (jj_scan_token(117)) return true;
    return false;
  }

  final private boolean jj_3R_124() {
    if (jj_scan_token(K_DIV)) return true;
    return false;
  }

  final private boolean jj_3R_149() {
    if (jj_scan_token(K_COUNT)) return true;
    if (jj_scan_token(115)) return true;
    Token xsp;
    xsp = jj_scanpos;
    if (jj_scan_token(5)) jj_scanpos = xsp;
    xsp = jj_scanpos;
    if (jj_scan_token(122)) {
    jj_scanpos = xsp;
    if (jj_3R_164()) return true;
    }
    if (jj_scan_token(117)) return true;
    return false;
  }

  final private boolean jj_3R_148() {
    if (jj_scan_token(K_NULL)) return true;
    return false;
  }

  final private boolean jj_3R_144() {
    Token xsp;
    xsp = jj_scanpos;
    if (jj_3R_148()) {
    jj_scanpos = xsp;
    if (jj_3R_149()) {
    jj_scanpos = xsp;
    if (jj_3R_150()) {
    jj_scanpos = xsp;
    if (jj_3R_151()) {
    jj_scanpos = xsp;
    if (jj_3R_152()) {
    jj_scanpos = xsp;
    if (jj_3R_153()) {
    jj_scanpos = xsp;
    if (jj_3R_154()) {
    jj_scanpos = xsp;
    if (jj_3R_155()) {
    jj_scanpos = xsp;
    if (jj_3R_156()) {
    jj_scanpos = xsp;
    if (jj_3R_157()) {
    jj_scanpos = xsp;
    if (jj_3R_158()) return true;
    }
    }
    }
    }
    }
    }
    }
    }
    }
    }
    return false;
  }

  final private boolean jj_3R_94() {
    Token xsp;
    xsp = jj_scanpos;
    if (jj_scan_token(6)) {
    jj_scanpos = xsp;
    if (jj_scan_token(125)) return true;
    }
    if (jj_3R_93()) return true;
    return false;
  }

  final private boolean jj_3R_143() {
    if (jj_scan_token(143)) return true;
    return false;
  }

  final private boolean jj_3R_78() {
    if (jj_3R_93()) return true;
    Token xsp;
    while (true) {
      xsp = jj_scanpos;
      if (jj_3R_94()) { jj_scanpos = xsp; break; }
    }
    return false;
  }

  final private boolean jj_3R_137() {
    Token xsp;
    xsp = jj_scanpos;
    if (jj_scan_token(144)) jj_scanpos = xsp;
    if (jj_3R_144()) return true;
    return false;
  }

  final private boolean jj_3R_79() {
    if (jj_scan_token(K_XOR)) return true;
    if (jj_3R_78()) return true;
    return false;
  }

  final private boolean jj_3R_123() {
    if (jj_scan_token(141)) return true;
    return false;
  }

  final private boolean jj_3R_66() {
    if (jj_3R_78()) return true;
    Token xsp;
    while (true) {
      xsp = jj_scanpos;
      if (jj_3R_79()) { jj_scanpos = xsp; break; }
    }
    return false;
  }

  final private boolean jj_3R_142() {
    if (jj_scan_token(139)) return true;
    return false;
  }

  public ZqlJJParserTokenManager token_source;
  SimpleCharStream jj_input_stream;
  public Token token, jj_nt;
  private int jj_ntk;
  private Token jj_scanpos, jj_lastpos;
  private int jj_la;
  public boolean lookingAhead = false;
  private boolean jj_semLA;
  private int jj_gen;
  final private int[] jj_la1 = new int[105];
  static private int[] jj_la1_0;
  static private int[] jj_la1_1;
  static private int[] jj_la1_2;
  static private int[] jj_la1_3;
  static private int[] jj_la1_4;
  static {
      jj_la1_0();
      jj_la1_1();
      jj_la1_2();
      jj_la1_3();
      jj_la1_4();
   }
   private static void jj_la1_0() {
      jj_la1_0 = new int[] {0x10000,0x0,0x0,0x213000,0x80000000,0x80000000,0x80000000,0x0,0x0,0x0,0x0,0x20000,0x0,0x0,0x0,0x20000,0x0,0x20000000,0x0,0x0,0x20000000,0x0,0x0,0x0,0x40708400,0x0,0x0,0x0,0x0,0x0,0x0,0x2000020,0x2000020,0x0,0x0,0x100,0x100,0x0,0x40708400,0x0,0x0,0x0,0x0,0x0,0x0,0x100,0x100,0x0,0x0,0x0,0x1000200,0x1000200,0x0,0x1000200,0x1000200,0x0,0x0,0x0,0x40,0x40,0x0,0x708400,0x0,0x700400,0x700400,0x0,0x8000000,0x0,0x0,0x700400,0x80,0xa0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x4000000,0x4000000,0x0,0x0,0x0,0x0,0x20,0x40708400,0x0,0x2000020,0x40708400,0x0,0x0,0x0,0x600400,0x600400,0x400000,0x2000020,0x2000020,0x42708420,0x0,};
   }
   private static void jj_la1_1() {
      jj_la1_1 = new int[] {0x22000201,0x0,0x0,0x22000201,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x8000000,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x155c8844,0x0,0x0,0x8,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x155c8844,0x0,0x0,0x8000,0x0,0x80000000,0x0,0x0,0x0,0x20,0x30,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x4000000,0x155c8844,0x4000000,0x115c8844,0x115c8844,0x0,0x0,0x4000000,0x0,0x115c8844,0x0,0x0,0x4000000,0x4000000,0x0,0x4000000,0x4000000,0x10000,0x0,0x0,0x0,0x0,0x0,0x0,0x100000,0x100000,0x0,0x0,0x0,0x0,0x0,0x155c8844,0x0,0x0,0x155c8844,0x4,0x10000000,0x804,0x15c8040,0x15c8040,0x1400040,0x0,0x0,0x155c8844,0x0,};
   }
   private static void jj_la1_2() {
      jj_la1_2 = new int[] {0x60000100,0x0,0x0,0x60000100,0x8040,0x40,0x8040,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x1,0x20000,0x8002000,0x8002000,0x22000,0x0,0x0,0x0,0x82204220,0x0,0x0,0x0,0x0,0x0,0x4,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x82204220,0x0,0x0,0x200,0x8,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x2,0x2,0x0,0x0,0x0,0x0,0x82204220,0x0,0x82204220,0x82204220,0x0,0x0,0x0,0x0,0x82204220,0x80000,0x80000,0x0,0x0,0x0,0x0,0x0,0x400,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x82204220,0x0,0x0,0x82204220,0x2000000,0x0,0x2000000,0x80204220,0x80204220,0x80004020,0x0,0x0,0x82204220,0x0,};
   }
   private static void jj_la1_3() {
      jj_la1_3 = new int[] {0x0,0x100000,0x80000,0x0,0x0,0x0,0x0,0x100000,0x8000,0x8000,0x8,0x0,0x100000,0x0,0x8,0x0,0x10,0x0,0x0,0x0,0x0,0x8000,0x2,0x100000,0xc80c0,0x100000,0x80000,0x0,0x2,0x8000,0x0,0x0,0x0,0x2,0x100000,0x0,0x8000,0x4000000,0xc80c0,0x88000,0x100000,0x0,0x0,0x0,0x88000,0x0,0x8000,0x0,0x0,0x100000,0x0,0x0,0x100000,0x0,0x0,0x10000000,0x10000000,0x20,0x20000000,0x20000000,0x0,0xc80c0,0x0,0xc80c0,0xc80c0,0x1,0x0,0x0,0xc1000000,0xc80c0,0x0,0x0,0x0,0x0,0x100000,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x4000000,0x4000000,0x0,0x0,0x0,0x0,0x0,0x40c80c0,0x100000,0x0,0xc80c0,0x0,0x0,0x48080,0x40,0x8040,0x40,0x0,0x0,0x40c80c0,0x2,};
   }
   private static void jj_la1_4() {
      jj_la1_4 = new int[] {0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x18c20,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x18c00,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x18c00,0x0,0x18c00,0x18c00,0x0,0x0,0x0,0x1f,0x18c00,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x40,0x80,0x300,0x300,0xc00,0xc00,0x3000,0x3000,0x4000,0x8c00,0x8c00,0x10000,0x0,0x18c00,0x0,0x0,0x18c00,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x18c00,0x0,};
   }
  final private JJCalls[] jj_2_rtns = new JJCalls[22];
  private boolean jj_rescan = false;
  private int jj_gc = 0;

  public ZqlJJParser(java.io.InputStream stream) {
     this(stream, null);
  }
  public ZqlJJParser(java.io.InputStream stream, String encoding) {
    try { jj_input_stream = new SimpleCharStream(stream, encoding, 1, 1); } catch(java.io.UnsupportedEncodingException e) { throw new RuntimeException(e); }
    token_source = new ZqlJJParserTokenManager(jj_input_stream);
    token = new Token();
    jj_ntk = -1;
    jj_gen = 0;
    for (int i = 0; i < 105; i++) jj_la1[i] = -1;
    for (int i = 0; i < jj_2_rtns.length; i++) jj_2_rtns[i] = new JJCalls();
  }

  public void ReInit(java.io.InputStream stream) {
     ReInit(stream, null);
  }
  public void ReInit(java.io.InputStream stream, String encoding) {
    try { jj_input_stream.ReInit(stream, encoding, 1, 1); } catch(java.io.UnsupportedEncodingException e) { throw new RuntimeException(e); }
    token_source.ReInit(jj_input_stream);
    token = new Token();
    jj_ntk = -1;
    jj_gen = 0;
    for (int i = 0; i < 105; i++) jj_la1[i] = -1;
    for (int i = 0; i < jj_2_rtns.length; i++) jj_2_rtns[i] = new JJCalls();
  }

  public ZqlJJParser(java.io.Reader stream) {
    jj_input_stream = new SimpleCharStream(stream, 1, 1);
    token_source = new ZqlJJParserTokenManager(jj_input_stream);
    token = new Token();
    jj_ntk = -1;
    jj_gen = 0;
    for (int i = 0; i < 105; i++) jj_la1[i] = -1;
    for (int i = 0; i < jj_2_rtns.length; i++) jj_2_rtns[i] = new JJCalls();
  }

  public void ReInit(java.io.Reader stream) {
    jj_input_stream.ReInit(stream, 1, 1);
    token_source.ReInit(jj_input_stream);
    token = new Token();
    jj_ntk = -1;
    jj_gen = 0;
    for (int i = 0; i < 105; i++) jj_la1[i] = -1;
    for (int i = 0; i < jj_2_rtns.length; i++) jj_2_rtns[i] = new JJCalls();
  }

  public ZqlJJParser(ZqlJJParserTokenManager tm) {
    token_source = tm;
    token = new Token();
    jj_ntk = -1;
    jj_gen = 0;
    for (int i = 0; i < 105; i++) jj_la1[i] = -1;
    for (int i = 0; i < jj_2_rtns.length; i++) jj_2_rtns[i] = new JJCalls();
  }

  public void ReInit(ZqlJJParserTokenManager tm) {
    token_source = tm;
    token = new Token();
    jj_ntk = -1;
    jj_gen = 0;
    for (int i = 0; i < 105; i++) jj_la1[i] = -1;
    for (int i = 0; i < jj_2_rtns.length; i++) jj_2_rtns[i] = new JJCalls();
  }

  final private Token jj_consume_token(int kind) throws ParseException {
    Token oldToken;
    if ((oldToken = token).next != null) token = token.next;
    else token = token.next = token_source.getNextToken();
    jj_ntk = -1;
    if (token.kind == kind) {
      jj_gen++;
      if (++jj_gc > 100) {
        jj_gc = 0;
        for (int i = 0; i < jj_2_rtns.length; i++) {
          JJCalls c = jj_2_rtns[i];
          while (c != null) {
            if (c.gen < jj_gen) c.first = null;
            c = c.next;
          }
        }
      }
      return token;
    }
    token = oldToken;
    jj_kind = kind;
    throw generateParseException();
  }

  static private final class LookaheadSuccess extends java.lang.Error { }
  final private LookaheadSuccess jj_ls = new LookaheadSuccess();
  final private boolean jj_scan_token(int kind) {
    if (jj_scanpos == jj_lastpos) {
      jj_la--;
      if (jj_scanpos.next == null) {
        jj_lastpos = jj_scanpos = jj_scanpos.next = token_source.getNextToken();
      } else {
        jj_lastpos = jj_scanpos = jj_scanpos.next;
      }
    } else {
      jj_scanpos = jj_scanpos.next;
    }
    if (jj_rescan) {
      int i = 0; Token tok = token;
      while (tok != null && tok != jj_scanpos) { i++; tok = tok.next; }
      if (tok != null) jj_add_error_token(kind, i);
    }
    if (jj_scanpos.kind != kind) return true;
    if (jj_la == 0 && jj_scanpos == jj_lastpos) throw jj_ls;
    return false;
  }

  final public Token getNextToken() {
    if (token.next != null) token = token.next;
    else token = token.next = token_source.getNextToken();
    jj_ntk = -1;
    jj_gen++;
    return token;
  }

  final public Token getToken(int index) {
    Token t = lookingAhead ? jj_scanpos : token;
    for (int i = 0; i < index; i++) {
      if (t.next != null) t = t.next;
      else t = t.next = token_source.getNextToken();
    }
    return t;
  }

  final private int jj_ntk() {
    if ((jj_nt=token.next) == null)
      return (jj_ntk = (token.next=token_source.getNextToken()).kind);
    else
      return (jj_ntk = jj_nt.kind);
  }

  private java.util.Vector jj_expentries = new java.util.Vector();
  private int[] jj_expentry;
  private int jj_kind = -1;
  private int[] jj_lasttokens = new int[100];
  private int jj_endpos;

  private void jj_add_error_token(int kind, int pos) {
    if (pos >= 100) return;
    if (pos == jj_endpos + 1) {
      jj_lasttokens[jj_endpos++] = kind;
    } else if (jj_endpos != 0) {
      jj_expentry = new int[jj_endpos];
      for (int i = 0; i < jj_endpos; i++) {
        jj_expentry[i] = jj_lasttokens[i];
      }
      boolean exists = false;
      for (java.util.Enumeration e = jj_expentries.elements(); e.hasMoreElements();) {
        int[] oldentry = (int[])(e.nextElement());
        if (oldentry.length == jj_expentry.length) {
          exists = true;
          for (int i = 0; i < jj_expentry.length; i++) {
            if (oldentry[i] != jj_expentry[i]) {
              exists = false;
              break;
            }
          }
          if (exists) break;
        }
      }
      if (!exists) jj_expentries.addElement(jj_expentry);
      if (pos != 0) jj_lasttokens[(jj_endpos = pos) - 1] = kind;
    }
  }

  public ParseException generateParseException() {
    jj_expentries.removeAllElements();
    boolean[] la1tokens = new boolean[145];
    if (jj_kind >= 0) {
      la1tokens[jj_kind] = true;
      jj_kind = -1;
    }
    for (int i = 0; i < 105; i++) {
      if (jj_la1[i] == jj_gen) {
        for (int j = 0; j < 32; j++) {
          if ((jj_la1_0[i] & (1<<j)) != 0) {
            la1tokens[j] = true;
          }
          if ((jj_la1_1[i] & (1<<j)) != 0) {
            la1tokens[32+j] = true;
          }
          if ((jj_la1_2[i] & (1<<j)) != 0) {
            la1tokens[64+j] = true;
          }
          if ((jj_la1_3[i] & (1<<j)) != 0) {
            la1tokens[96+j] = true;
          }
          if ((jj_la1_4[i] & (1<<j)) != 0) {
            la1tokens[128+j] = true;
          }
        }
      }
    }
    for (int i = 0; i < 145; i++) {
      if (la1tokens[i]) {
        jj_expentry = new int[1];
        jj_expentry[0] = i;
        jj_expentries.addElement(jj_expentry);
      }
    }
    jj_endpos = 0;
    jj_rescan_token();
    jj_add_error_token(0, 0);
    int[][] exptokseq = new int[jj_expentries.size()][];
    for (int i = 0; i < jj_expentries.size(); i++) {
      exptokseq[i] = (int[])jj_expentries.elementAt(i);
    }
    return new ParseException(token, exptokseq, tokenImage);
  }

  final public void enable_tracing() {
  }

  final public void disable_tracing() {
  }

  final private void jj_rescan_token() {
    jj_rescan = true;
    for (int i = 0; i < 22; i++) {
    try {
      JJCalls p = jj_2_rtns[i];
      do {
        if (p.gen > jj_gen) {
          jj_la = p.arg; jj_lastpos = jj_scanpos = p.first;
          switch (i) {
            case 0: jj_3_1(); break;
            case 1: jj_3_2(); break;
            case 2: jj_3_3(); break;
            case 3: jj_3_4(); break;
            case 4: jj_3_5(); break;
            case 5: jj_3_6(); break;
            case 6: jj_3_7(); break;
            case 7: jj_3_8(); break;
            case 8: jj_3_9(); break;
            case 9: jj_3_10(); break;
            case 10: jj_3_11(); break;
            case 11: jj_3_12(); break;
            case 12: jj_3_13(); break;
            case 13: jj_3_14(); break;
            case 14: jj_3_15(); break;
            case 15: jj_3_16(); break;
            case 16: jj_3_17(); break;
            case 17: jj_3_18(); break;
            case 18: jj_3_19(); break;
            case 19: jj_3_20(); break;
            case 20: jj_3_21(); break;
            case 21: jj_3_22(); break;
          }
        }
        p = p.next;
      } while (p != null);
      } catch(LookaheadSuccess ls) { }
    }
    jj_rescan = false;
  }

  final private void jj_save(int index, int xla) {
    JJCalls p = jj_2_rtns[index];
    while (p.gen > jj_gen) {
      if (p.next == null) { p = p.next = new JJCalls(); break; }
      p = p.next;
    }
    p.gen = jj_gen + xla - jj_la; p.first = token; p.arg = xla;
  }

  static final class JJCalls {
    int gen;
    Token first;
    int arg;
    JJCalls next;
  }

    // main ends here

}
