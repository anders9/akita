...
public class ZUdf {
	...
	public int getParmN() {return parm.length; }
	public boolean isAggr() {return isAggr; }
	public String getParm(int i) {return parm[i]; }
	public boolean getParmAggr(int i) {return toAggr[i]; }
		...
		re.traverse(new NodeVisitor(){
			...
			{	
				if(node instanceof ZColRef){
					...
					if(c.table != null || c.col == null || !parm.contains(c.col))
						throw new ExecException("Invalid UDF function definition: col '"
								+ c.toString() + "' in: " + ZUdf.this.exp);
					int aggrCnt = 0;
					while(node != root){
					...
					if(aggrCnt > 1)
						throw new ExecException("Invalid UDF function definition: col '"
								+ c.toString() + "' is aggregated more than once in: "
								+ ZUdf.this.exp);
